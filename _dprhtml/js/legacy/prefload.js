const DPR_Prefload = (() => {
	...
	DPR_G.DPR_prefsinfo = {
		...
		imgbk: {
			type: String.name,
			defaultValue: "url(/images/imgbk.png)",
		},
	...
	DPR_G.DPR_prefsinfo[`${searchSettingsKeyName}`] = {
		type: String.name,
		defaultValue:
			'{"type":0,"query":"","MAT":"m","set":"dmsak","book":"1","part":1,"rx":false}',
	};

	DPR_G.DPR_prefsinfo[`${dictionarySearchSettingsKeyName}`] = {
		type: String.name,
		defaultValue:
			'{"type":"DPR","query":"","opts":"xv,xd,xm,xs,xa,xk,xy,mm,ma,mt,hd","entry":""}',
	};
	...
	DPR_G.DPR_prefsD = Object.entries(DPR_G.DPR_prefsinfo).reduce(
		(acc, [k, v]) => {
			acc[k] = v.defaultValue;
			return acc;
		},
		[],
	);
	DPR_G.DPR_prefTypesD = Object.entries(DPR_G.DPR_prefsinfo).reduce(
		(acc, [k, v]) => {
			acc[k] = v.type;
			return acc;
		},
	...
	function loadPreferences() {
		Object.entries(DPR_G.DPR_prefsinfo).reduce(
			(acc, [k, _]) => ((acc[k] = getPref(k)), acc),
			DPR_G.DPR_prefs,
		);
	}

	function savePreferences(getPrefFn) {
		Object.entries(DPR_G.DPR_prefsinfo).forEach(([k, v]) => {
	...
	function getPref(name) {
		...
		const prefType = DPR_G.DPR_prefTypesD[name];

		const prefStorageKey = getPrefStorageKey(name);
		if (!(localStorage[prefStorageKey] === undefined)) {
			switch (prefType) {
				case Number.name:
					pref = Number.parseInt(localStorage[prefStorageKey]);
					break;
				case String.name:
	...
	function resetAllDprSettings() {
		Object.entries(localStorage).forEach(([k, _]) =>
	...
	const saveDictionarySearchSettings = (value) =>
		setPref(dictionarySearchSettingsKeyName, value);
	const loadDictionarySearchSettings = () =>
		getPref(dictionarySearchSettingsKeyName);
