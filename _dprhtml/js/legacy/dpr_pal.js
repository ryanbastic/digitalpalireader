((DPR_Pal, $, undefined) => {
	const defineReadOnlyProperty = (name, value) =>
		Object.defineProperty(DPR_Pal, name, { value: value });
	...
	defineReadOnlyProperty("isWeb", !DPR_Pal.isXUL);
	...
	defineReadOnlyProperty(
		"dprHomePage",
		DPR_Pal.isXUL
			? `${DPR_Pal.baseUrl}digitalpalireader/content/index.xul`
			: `${DPR_Pal.baseUrl}index.html`,
	);
  ...
  */
	...
	DPR_Pal.copyToClipboard = (text) => {
		if (DPR_Pal.isWeb) {
			var targetElement = $(".modal-open").length
				? $(".modal-body")
				: $("body");
			$("<input>")
				.attr("class", "clipboardCopy")
				.attr("style", "position: absolute; left: -1000px; top: -1000px")
				.val(text)
	...
	DPR_Pal.normalizeDprUri = (uri) => {
		if (DPR_Pal.isWeb && uri.match(dprSchemeUriCracker)) {
			return uri.replace(dprSchemeUriCracker, `${DPR_Pal.baseUrl}$1.html?$2`);
		} else {
			return uri;
		}
	};
	...
	DPR_Pal.dprUrlMatcher = /\/index\.html/;
	...
	DPR_Pal.getDifId = () =>
		DPR_Pal.isDictionaryFeature() && !DPR_Pal.isDictionaryAnalysisFeature()
			? "difb"
			: "difb-bottom";
	...
	DPR_Pal.DPR_initialTabs = Object.freeze({
		"DPR-main": {
			test: (x) =>
				!x.includes("?feature=") ||
				/\?feature=search&/i.test(x) ||
				/\?feature=dictionary&/i.test(x),
		},
	...
	DPR_Pal.updateMostRecentUrl = () =>
		(DPR_Pal.mostRecentUrl = document.location.href);

	DPR_Pal.isNavigationFeature = () => /\?loc=/i.exec(document.location.href);

	DPR_Pal.isSearchFeature = () =>
		/\?feature=search&/i.exec(document.location.href);

	DPR_Pal.isDictionaryFeature = () =>
		/\?feature=dictionary&/i.exec(document.location.href);

	DPR_Pal.isDictionaryAnalysisFeature = () =>
		/&analysis=/i.exec(document.location.href) &&
		document.location.href != DPR_Pal.mostRecentUrl;

	DPR_Pal.getTranslationsBaseUrl = () =>
		`${/localdev/i.test(window.environmentName) ? "https://staging.digitalpalireader.online" : ""}/_external/translations`;
	...
	DPR_Pal.toWebUrl = (url) => {
		const xulUrlCracker =
			/^chrome:\/\/digitalpalireader\/content\/(index|search|dict|dbv)\.(xul|htm|html)(\?)?(.*)$/i;

		let newUrl = url
			.replace(xulUrlCracker, `${DPR_Pal.baseUrl}index.html?feature=$1&$4`)
	...
	Object.defineProperty(DPR_Pal, "mainWindow", {
		get() {
			if (!_mainWindow) {
	...
	Object.defineProperty(DPR_Pal, "contentWindow", {
		get() {
			return DPR_Pal.mainWindow.gBrowser.selectedTab.linkedBrowser
				.contentWindow;
		},
		enumerable: true,
		configurable: true,
	...
	Object.defineProperty(DPR_Pal, "contentDocument", {
		get() {
			return DPR_Pal.mainWindow.gBrowser.selectedTab.linkedBrowser
				.contentDocument;
		},
	...
	DPR_Pal.getId = (url) => {
		var tabDef = Object.entries(DPR_Pal.DPR_initialTabs).find(([_, x]) =>
			x.test(url),
		);
		if (tabDef) {
			return tabDef[0];
	...
	DPR_Pal.createMainWindow = (window) => {
		const gBrowser = {
			...
			moveTabTo(tab, index) {
				const otherTabs = this.tabContainer.childNodes.filter(
					(t) => t._window !== tab._window,
				);
			...
			_selectedTab: null,
			get selectedTab() {
				const tab = this.tabContainer.childNodes.find(
					(t) => t._window.DPR_Pal.windowHasFocus,
				);
			...
			get tabContainer() {
				return {
					childNodes: [...this._childNodes.filter((x) => !x._window.closed)],
				};
			},
		};
