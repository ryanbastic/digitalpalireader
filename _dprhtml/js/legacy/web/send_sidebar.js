const DPR_Web_Send_Sidebar = (() => {
	...
	async function importXML(section, labelsearch, para, isPL, add, scroll, cat) {
		...
		var bookno = Number.parseInt(document.getElementById("nav-book").value) - 1;
		...
		if (DPR_G.G_hier == "t" && DPRNav.limitt()) {
			DPR1_format_mod.alertFlash(
				"Ṭīkā not available for " +
					DPR_G.G_nikLongName[document.getElementById("set").value] +
					".",
				"RGBa(255,0,0,0.8)",
			);
			return;
		...
		if (DPR_G.G_hier == "a" && nikaya == "g") {
			DPR1_format_mod.alertFlash(
				"Atthakatha not available for grammar.",
				"RGBa(255,0,0,0.8)",
			);
			return;
		}
		if (DPR_G.G_hier == "a" && nikaya == "b") {
			DPR1_format_mod.alertFlash(
				"Atthakatha not available for Abhidh-s.",
				"RGBa(255,0,0,0.8)",
			);
			return;
		}

		const loadXmlSectionParams = [
			nikaya,
			bookno,
			meta,
			volume,
		...
		if (!add) {
			...
			if (thisTab) {
				var thisTabBrowser =
					DPR_PAL.mainWindow.gBrowser.getBrowserForTab(thisTab);
				await thisTabBrowser.contentDocument
					.getElementById("dpr-tops")
					.getElementsByTagName("browser")[0]
					.contentWindow.DPR_Chrome.addOrOpenMainPanelSection(
						DPR_Translations.makeDprPlace(loadXmlSectionParams),
						false,
						sectionId,
						labelsearch,
						para,
			...
			if (!oldTab) {
				var permalink = DPR_PAL.toWebUrl(
					"chrome://digitalpalireader/content/index.xul" +
						"?loc=" +
						nikaya +
						"." +
						bookno +
						...
						"." +
						DPR_G.G_hier +
						(labelsearch ? "&query=" + labelsearch.join("+") : "") +
						(para ? "&para=" + para : "") +
				...
				var oldTabBrowser =
					DPR_PAL.mainWindow.gBrowser.getBrowserForTab(oldTab);
				await oldTabBrowser.contentDocument
					.getElementById("dpr-tops")
					.getElementsByTagName("browser")[0]
					.contentWindow.DPR_Chrome.addOrOpenMainPanelSection(
						DPR_Translations.makeDprPlace(loadXmlSectionParams),
		...
		if (!add) {
			...
			if (thisTab) {
				...
				await thisTabBrowser.contentDocument
					...
					.getElementsByTagName("browser")[0]
					.contentWindow.DPR_Chrome.addOrOpenMainPanelSection(
						DPR_Translations.makeDprPlace(loadXmlIndexParams),
						false,
						sectionId,
					);
				return;
	...
	async function openPlace(
		sectionId,
		[nikaya, book, meta, volume, vagga, sutta, section, hiert],
		para,
		stringra,
		add,
	) {
		...
		if (stringra) {
			...
			if (DPR_G.G_searchRX == "true") {
				for (var i in stringra) {
					stringra[i] = new RegExp(stringra[i]);
				}
			}
		}
		if (!add) {
			...
			if (!oldTab) {
				var permalink = DPR_PAL.toWebUrl(
					"chrome://digitalpalireader/content/index.xul" +
						...
						hiert +
						(stringra
							? "&query=" + DPR_translit_mod.toVel(stringra.join("+"))
							: "") +
						(para ? "&para=" + (para + 1) : ""),
				);
				DPRChrome.openDPRTab(permalink, "DPR-main");
		...
		if (!add) {
			...
			if (thisTab) {
				...
				await thisTabBrowser.contentDocument
					...
					.getElementsByTagName("browser")[0]
					.contentWindow.DPR_Chrome.addOrOpenMainPanelSection(
						DPR_Translations.makeDprPlace([nikaya, book, hiert]),
						false,
						sectionId,
					);
				return;
	...
	async function sendQuickLink(add, value) {
		if (add == "right") return;
		var ql = DPR_navigation_common_mod.convertShortLink(
			$.trim($("#nav-quicklinks").val()),
		);
		if (!ql) return;
		if (ql[0] === false) {
		...
		if (!add) {
			...
			if (thisTab) {
				...
				await thisTabBrowser.contentDocument
					.getElementById("dpr-tops")
					.getElementsByTagName("browser")[0]
					.contentWindow.DPR1_send_mod.openPlace(
						sectionId,
						ql,
						para,
						null,
			...
			if (thisTab) {
				...
				await thisTabBrowser.contentDocument
					.getElementById("dpr-tops")
					.getElementsByTagName("browser")[0]
					.contentWindow.DPR1_send_mod.openPlace(null, ql, para, null, add);
				return;
			}
			var oldTab = DPRChrome.findDPRTab("DPR-main");
			if (!oldTab) {
	...
	async function sendAnalysisToOutput(sectionId, input, frombox, add) {
		...
		if (!add) {
			...
			if (thisTab) {
				var thisTabBrowser =
					DPR_PAL.mainWindow.gBrowser.getBrowserForTab(thisTab);
				await thisTabBrowser.contentWindow.DPR1_analysis_function_mod.outputAnalysis(
					sectionId,
					input,
					frombox,
				);
			...
			if (!oldTab) {
				...
				var oldTabBrowser =
					DPR_PAL.mainWindow.gBrowser.getBrowserForTab(oldTab);
				await oldTabBrowser.contentWindow.DPR1_analysis_function_mod.outputAnalysis(
					sectionId,
					input,
					frombox,
				);
	...
	async function sendDict(hard, add, which, getstring, opts) {
		...
		if (!getstring) {
			...
			if (!hard) {
				if (
					getstring == DPR_G.G_lastsearch ||
					getstring == "" ||
					!DPR_G.DPR_prefs["autodict"] ||
					$("#soregexp").prop("checked") ||
					$("#sofulltext").prop("checked")
				)
					return;
			}
			...
			if (hard)
				DPR_search_history_mod.saveDictHistory(
					getstring,
					which,
					opts.join(","),
				);
		...
		if (which == "DPR") {
			...
			DPR_prefload_mod.saveDictionarySearchSettings(
				JSON.stringify({
					type: which,
					query: getstring,
					opts: opts.join(","),
			...
			if (!add) {
				...
				if (!oldTab) {
					...
					var oldTabBrowser =
						DPR_PAL.mainWindow.gBrowser.getBrowserForTab(oldTab);
					await oldTabBrowser.contentWindow.DPR_dict_mod.startDictLookup(
						sectionId,
						which,
						getstring,
						opts,
	...
	async function sendSearch(
		...
		if (!searchString) {
			...
			if (getstring == "_dev") {
				// Dev
				DPRChrome.openDPRTab(
					DPR_PAL.toWebUrl("chrome://digitalpalireader/content/dev.xul"),
					"DPRd",
				);
				return;
			}
			...
			if (which == 0 || which == 2) {
				var MAT =
					(document.getElementById("tsoMATm").checked ? "m" : "") +
					(document.getElementById("tsoMATa").checked ? "a" : "") +
					(document.getElementById("tsoMATt").checked ? "t" : "");
			...
			if (which == 0) {
				...
				var sets = "";
				for (var i in "dmsak") {
					if (document.getElementById("tsoCO" + "dmsak"[i]).checked)
						sets += "dmsak"[i];
				}
			...
			if (which == 1) {
				...
				if (DPR_G.nikvoladi[document.getElementById("tsoSETm").value]) {
					for (
						var i = 1;
						i <=
						DPR_G.nikvoladi[document.getElementById("tsoSETm").value].length;
						i++
					) {
					...
					for (
						...
						i <=
						DPR_G.nikvoladi[
							document.getElementById("tsoSETm").value +
								document.getElementById("tsoMAT2m").value
						].length;
						i++
					) {
						if (document.getElementById("tsoBObook" + i).checked)
							book.push(
								document
									.getElementById("tsoBObook" + i)
			...
			if (which == 3) {
				// get parts
				var part =
					document.getElementById("tsoPR").value -
					1 +
					"." +
					$("#tsoPmeta option:selected").index() +
					"." +
					$("#tsoPvolume option:selected").index() +
					"." +
					$("#tsoPvagga option:selected").index() +
					"." +
					$("#tsoPsutta option:selected").index() +
					"." +
					$("#tsoPsection option:selected").index();
			} else part = 1;

			var rx = document.getElementById("tsoRx").checked;

			DPR_search_history_mod.addSearchHistory(
				getstring,
				which,
				rx,
				sets,
		...
		DPR_prefload_mod.saveSearchSettings(
			JSON.stringify({
				type: which,
				query: getstring,
				MAT,
		...
		if (!add) {
			...
			if (!oldTab) {
				...
				var oldTabBrowser =
					DPR_PAL.mainWindow.gBrowser.getBrowserForTab(oldTab);
				await oldTabBrowser.contentDocument
					.getElementById("dpr-search-browser")
					.contentWindow.DPR1_search_mod.searchTipitaka(
						sectionId,
						which,
						getstring,
						MAT,
	...
	function checkGetstring(getstring) {
		...
		if (getstring.length < 3) {
			DPR1_format_mod.alertFlash(
				"Minimum three letter search length",
				"yellow",
			);
			$("#sbfb").html(
				'<div align = center><br><br><br><br><br><h1 id = "c">ready</h1></div>',
			);
			$("#sbfa").html("");
			$("#sbfab").html("");
			return false;
