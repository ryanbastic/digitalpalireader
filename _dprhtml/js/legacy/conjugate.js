const DPR_Conjugate = (() => {
	async function insertConj() {
		...
		var conjUrl =
			"digitalpalireader/content/conjugate.htm" +
			(word.length ? "?word=" + DPR_translit_mod.toVel(word) : "") +
			(form.length
				? (word ? "&" : "?") + "form=" + DPR_translit_mod.toVel(form)
				: "");
		...
		$("#conjugation").html(
			out ? out : '<span class="red">no conjugation found</span>',
		);
	}
