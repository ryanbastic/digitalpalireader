const DPR_Dict = (() => {
	...
	function parseDictURLParameters() {
		const urlDictionarySearchParams = new URLSearchParams(
			DPR_PAL.isDictionaryFeature() ? window.location.search : "",
		);
		const savedDictionarySearchParams = JSON.parse(
			DPR_prefload_mod.loadDictionarySearchSettings(),
		);
		const getDictionarySearchParamValue = (n) =>
			urlDictionarySearchParams.get(n) || savedDictionarySearchParams[n];

		window.DPR_Globals.DictionaryTabViewModel.type(
			(DPR_G.G_dictType = getDictionarySearchParamValue("type")),
		);
		window.DPR_Globals.DictionaryTabViewModel.query(
			(DPR_G.G_dictQuery = decodeURIComponent(
				getDictionarySearchParamValue("query"),
			)),
		);
		window.DPR_Globals.DictionaryTabViewModel.options(
			(DPR_G.G_dictOpts = getDictionarySearchParamValue("opts").split(",")),
		);
		window.DPR_Globals.DictionaryTabViewModel.entry(
			(DPR_G.G_dictEntry = decodeURIComponent(
				getDictionarySearchParamValue("entry"),
			)),
		);
	}

	// TODO: Find out when this is triggered, because the import statement wouldn't work
	async function startDictLookup(
		sectionId,
		dictType,
		dictQuery,
		dictOpts,
		...
		if (dictType) {
			...
			var permalink = DPR_PAL.toWebUrl(
				"chrome://digitalpalireader/content/dict.htm" +
					"?type=" +
					DPR_G.G_dictType +
					(DPR_G.G_dictQuery ? "&query=" + DPR_G.G_dictQuery : "") +
					"&opts=" +
					DPR_G.G_dictOpts.join(",") +
					(DPR_G.G_dictEntry ? "&entry=" + DPR_G.G_dictEntry : ""),
			);
			try {
		...
		await import("./" + js[DPR_G.G_dictType]);
		...
		var tabT =
			...
			(DPR_G.G_dictQuery != ""
				? DPR_G.G_dictQuery
				: DPR_translit_mod.toUni(DPR_G.G_dictEntry.split(",")[1])) +
			"' in " +
			st[DPR_G.G_dictType];
		...
		$(
			`${DPR_Chrome.getSectionElementId(sectionId)} #main-pane-container-section-header`,
		).html(
			'<span style="float:left" title="Click to copy permalink to clipboard" onclick="DPR1_format_mod.permalinkClick(\'' +
				`${DPR_PAL.dprHomePage}?feature=dictionary&type=` +
				DPR_G.G_dictType +
				(DPR_G.G_dictQuery
					? "&query=" +
						DPR_translit_mod.toUni(DPR_G.G_dictQuery.replace(/ /g, "_"))
					: "") +
				"&opts=" +
				DPR_G.G_dictOpts.join(",") +
				(DPR_G.G_dictEntry
					? "&entry=" +
						DPR_translit_mod.toUni(DPR_G.G_dictEntry.replace(/ /g, "_"))
					: "") +
	...
	async function pedsearchstart(sectionId, hard) {
		...
		if (getstring == "") {
			await DPR1_dict_xml_mod.paliXML(
				DPR_translit_mod.toUni(DPR_G.G_dictEntry),
			);
			return;
		}

		if (!/[^0-9\/]/.exec(getstring) && DPR_G.devCheck == 1) {
			// dev link
		...
		for (var pedt in DPR_G.P) {
			...
			if (/rx/.exec(DPR_G.G_dictOpts)) {
				// reg exp
				var yessir =
					totest.search(getstring) == 0 ||
					(!/sw/.test(DPR_G.G_dictOpts) && totest.search(getstring) > -1);
			} else {
				// non reg exp
				var yessir =
					totest.indexOf(getstring) == 0 ||
					(!/sw/.test(DPR_G.G_dictOpts) && totest.indexOf(getstring) > -1);
			}
			if (yessir) {
				for (var z = 0; z < DPR_G.P[pedt].length; z++) {
					...
					finouta[y] =
						...
						DPR_G.DPR_prefs["coltext"] +
						'" onclick="DPR1_dict_xml_mod.paliXML(\'PED/' +
						loc +
						"," +
						uniout +
						"');\">" +
						uniout +
						(DPR_G.P[pedt].length > 1 ? " " + (z + 1) : "") +
						"</a><br>";
		...
		if (finouta.length == 0) {
			...
			if (/hd/.exec(DPR_G.G_dictOpts) || hard) {
				// find similar words if hard search
				var simlist = DPR_sortaz_mod.findSimilarWords(
					DPR_translit_mod.toFuzzy(getstring),
					DPR_G.P,
					DPR_G.G_similar_min,
					1,
				);
				...
				finouta.push(
					'<a href="javascript:void(0)" style="color:' +
						DPR_G.DPR_prefs["colped"] +
						'" onclick="DPR_dict_mod.pedsearchstart(DPR_Chrome.getPrimarySectionId(), 1)">Show Suggestions</a><br>',
				);
			}
		...
		if (DPR_G.G_dictEntry)
			DPR1_dict_xml_mod.paliXML(DPR_translit_mod.toUni(DPR_G.G_dictEntry));
	...
	async function pedFullTextSearch(sectionId, getstring) {
		...
		$(
			...
			'<div><a name="diftop">PED full-text search for <b style="color:' +
				DPR_G.DPR_prefs["colped"] +
				'">' +
				(/rx/.exec(DPR_G.G_dictOpts)
					? DPR_translit_mod.toUniRegEx(getstring)
		...
		if (y == 0) {
			var outDiv = document.createElement("div");
			outDiv.innerHTML =
				listoutf + "<tr><td>No results</td></tr></table><hr />";
			$("#dict").html("");
			document.getElementById("dict").appendChild(outDiv);
			document.getElementById("odif").scrollTop = 0;
			return;
	...
	async function dppnsearchstart(sectionId, hard) {
		...
		if (getstring == "") {
			await DPR1_dict_xml_mod.DPPNXML(
				DPR_translit_mod.toUni(DPR_G.G_dictEntry),
			);
			return;
		}
			...
			if (yessir) {
				for (var z = 0; z < DPR_G.D[x].length; z++) {
					...
					finouta.push(
						'<a href="javascript:void(0)" style="color:' +
							DPR_G.DPR_prefs["coltext"] +
							'" onClick="DPR1_dict_xml_mod.DPPNXML(\'' +
							uniout +
							"/" +
							loc +
							"," +
		...
		if (finouta.length == 0) {
			...
			if (/hd/.exec(DPR_G.G_dictOpts) || hard) {
				...
				finouta.push(
					'<a href="javascript:void(0)" style="color:' +
						DPR_G.DPR_prefs["colped"] +
						'" onclick="DPR_dict_mod.dppnsearchstart(DPR_Chrome.getPrimarySectionId(), 1)">Show Suggestions</a><br>',
				);
			}
		}
	...
	async function dppnFullTextSearch(sectionId, getstring) {
		...
		for (var i = 1; i < 10; i++) {
			...
			for (var j = 0; j < allp.length; j++) {
				...
				if (startmatch >= 0) {
					...
					const scrollTopElem = DPR_PAL.isXUL ? "dictc" : "paliTextContent";
					...
					finalouta.push(
						ttitle +
							...
							"/" +
							j +
							'"><div style="position:relative"><div style="position:absolute;top:0px; left:0px;"><a href="javascript:void(0)" onclick="DPR1_chrome_mod.scrollMainPane(0);" class="small" style="color:' +
							DPR_G.DPR_prefs["colped"] +
							'">top</a></div><br/>' +
		...
		$(
			...
			'<div><a name="diftop">DPPN full-text search for <b style="color:' +
				DPR_G.DPR_prefs["colped"] +
				'">' +
				getstring +
				"</b>:</div>" +
		...
		for (var x = 0; x < DPR_G.G_cpedAlt.length; x++) {
			...
			if (/rx/.exec(DPR_G.G_dictOpts)) {
				...
				var yessir =
					...
					(!/sw/.test(DPR_G.G_dictOpts) && tosearch.search(getstring) > -1);
			} else {
				// non reg exp
				var yessir =
					tosearch.indexOf(getstring) == 0 ||
					(!/sw/.test(DPR_G.G_dictOpts) && tosearch.indexOf(getstring) > -1);
			}
			if (yessir) {
				cnt++;
				...
				finouta.push(
					'<div><b><a style="color:' +
						...
						cnt +
						"').innerHTML == '') { DPR_grammar_mod.conjugate('" +
						us +
						"','cped" +
						cnt +
		...
		$(
			...
			'<p>CPED search for <b style="color:' +
				...
				(/rx/.exec(DPR_G.G_dictOpts)
					? DPR_translit_mod.toUniRegEx(getstring)
					: DPR_translit_mod.toUni(getstring)) +
				'</b>:<table width=100%><tr><td valign="top">',
		);
			...
			if (/hd/.exec(DPR_G.G_dictOpts) || hard) {
				...
				if (simlist) {
					...
					for (var i in simlist) {
						...
						ud = DPR_translit_mod.toUni(
							DPR_G.yt[pedt][2] + " (" + DPR_G.yt[pedt][1] + ")",
						);

						finouta.push(
							'<div><b><a style="color:' +
								DPR_G.DPR_prefs["colcpd"] +
				...
				finouta.push(
					'<a href="javascript:void(0)" style="color:' +
						DPR_G.DPR_prefs["colcpd"] +
						'" onclick="DPR_dict_mod.mlsearchstart(DPR_Chrome.getPrimarySectionId(),1)">Show Suggestions</a><br>',
				);
			}
		}
	...
	function multisearchstart(sectionId, hard) {
				...
				for (var z = 0; z < DPR_G.P[pedt].length; z++) {
					...
					finouta.push(
						uniout +
							...
							uniout +
							"')\">" +
							DPR_translit_mod.translit(uniout) +
							(DPR_G.P[pedt].length > 1 ? " " + (z + 1) : "") +
							"</a><br>",
					);
				}
		...
		for (var x = 0; x < DPR_G.G_cpedAlt.length; x++) {
			...
			if (/rx/.exec(DPR_G.G_dictOpts)) {
				...
				var yessir =
					tosearch.search(getstring) == 0 ||
					(!/sw/.exec(DPR_G.G_dictOpts) && tosearch.search(getstring) > -1);
			} else {
				// non reg exp
				var yessir =
					tosearch.indexOf(getstring) == 0 ||
					(!/sw/.exec(DPR_G.G_dictOpts) && tosearch.indexOf(getstring) > -1);
			}
			if (yessir) {
				cnt++;
				us = DPR_translit_mod.toUni(gsplit[0]);
				...
				finouta.push(
					us +
						'###<div><a style="color:' +
						DPR_G.DPR_prefs["colcpd"] +
						'" href="javascript:void(0)" onclick=" DPR_grammar_mod.conjugate(\'' +
						us +
						"','dif')\" title=\"" +
						ud.replace(/"/g, "&amp;quot;") +
						'">' +
		...
		if (finouta.length == 0) {
			...
			if (/hd/.exec(DPR_G.G_dictOpts) || hard) {
				// find similar words if hard search
				var simlistp = DPR_sortaz_mod.findSimilarWords(
					DPR_translit_mod.toFuzzy(getstring),
					DPR_G.P,
					DPR_G.G_similar_min,
					1,
				);
				var simlistd = DPR_sortaz_mod.findSimilarWords(
					DPR_translit_mod.toFuzzy(getstring),
					DPR_G.D,
					DPR_G.G_similar_min,
					1,
				);
				var simlistc = DPR_sortaz_mod.findSimilarWords(
					DPR_translit_mod.toFuzzy(getstring),
					DPR_G.yt,
					DPR_G.G_similar_min,
					1,
	...
	function attsearchstart(sectionId) {
		...
		for (var x = 0; x < DPR_G.attlist.length; x++) {
			...
			if (yessir) {
				...
				for (var a = 0; a < entries.length; a++) {
					...
					if (DPR_G.G_dictOpts.indexOf("x" + tnik) == -1)
						entries.splice(a--, 1);
				...
				finouta[y] =
					...
					DPR_G.DPR_prefs["coltext"] +
					'" onclick="DPR1_dict_xml_mod.getAtthXML(' +
					x +
					",'a','" +
					outnik +
					"')\">" +
		...
		$(
			...
			'<p>Aṭṭhakathā term search for <b style="color:' +
				DPR_G.DPR_prefs["colped"] +
				'">' +
				DPR_translit_mod.translit(DPR_translit_mod.toUni(getstring)) +
				"</b>:",
		);
	...
	function titlesearchstart(sectionId) {
		...
		for (var x = 0; x < DPR_G.titlelist.length; x++) {
			...
			if (yessir) {
				...
				for (var a = 0; a < entries.length; a++) {
					if (
						!DPR_G.G_dictOpts.indexOf(
							"m" + entries[a].charAt(entries[a].length - 3),
						)
					) {
						entries.splice(a--, 1);
					}
				...
				if (dppnEntry.length > 0) {
					for (var d in dppnEntry) {
						dEI +=
							'&nbsp;<span class="pointer" style="color:' +
							DPR_G.DPR_prefs["coldppn"] +
							'" title="DPPN entry" onclick="DPR1_dict_xml_mod.toggleDppnTitle(\'' +
							dppnEntry[d] +
							"','titleS" +
							x +
							"^" +
							...
							"');\">n</span>";
						dEO +=
							'<div class="hide round" id="titleS' + x + "^" + d + '"></div>';
					}
				}
				finouta.push(
					'<span class="pointer" style="color:' +
						DPR_G.DPR_prefs["coltext"] +
						'" onclick="DPR1_dict_xml_mod.getTitleXML(' +
						x +
						"," +
						(DPR_G.G_dictOpts.indexOf("mm") > -1) +
						"," +
						(DPR_G.G_dictOpts.indexOf("ma") > -1) +
						"," +
						(DPR_G.G_dictOpts.indexOf("mt") > -1) +
						",'" +
						outnik +
						"')\">" +
						uniout +
			...
			if (yessir) {
				...
				finouta.push(
					'<b><font style="color:' +
						...
						"</font></b>: " +
						gsplit[1] +
						' <span class="pointer hoverShow" title="go to entry in Dhātumālā" onclick="DPR1_send_mod.openPlace(' +
						`${sectionId}, ` +
						"['g',3," +
						ln[0] +
						",0," +
						ln[1] +
						",0,0,'m']," +
						(Number.parseInt(ln[2]) + 1) +
						',null,DPR1_send_mod.eventSend(event))">&rarr;</span><br>',
				);
			}
		}
	...
	async function sktsearchstart(sectionId) {
		...
		var getstring = DPR_translit_mod.toSkt(
			DPR_translit_mod.toVel(DPR_G.G_dictQuery),
		);
		if (/fz/.exec(DPR_G.G_dictOpts)) {
			getstring = DPR_translit_mod.toFuzzy(getstring);
		}
		...
		for (var x = 0; x < DPR_G.skt[char].length; x++) {
			...
			if (yessir) {
				...
				finouta[y] =
					'<span class="pointer" style="color:' +
					DPR_G.DPR_prefs["coltext"] +
					'" onclick="DPR1_dict_xml_mod.sktXML(\'' +
					sx +
					"'," +
					x +
					');">' +
		...
		$(
			...
			'<p>Sanskrit search for <b style="color:' +
				...
				'">' +
				(/rx/.exec(DPR_G.G_dictOpts)
					? DPR_translit_mod.toUniRegEx(DPR_G.G_dictQuery)
					: DPR_translit_mod.toUni(DPR_G.G_dictQuery)) +
				"</b>:",
		);
			...
			if (/hd/.exec(DPR_G.G_dictOpts) || hard) {
				...
				if (simlist) {
					...
					for (var x = 0; x < simlist.length; x++) {
						sx = simlist[x][1];
						var uniout = DPR_translit_mod.toUni(
							DPR_translit_mod.toSkt(sx, true),
						);
						finouta[y] =
							'<a href="javascript:void(0)" style="color:' +
							DPR_G.DPR_prefs["coltext"] +
		...
		if (DPR_G.G_dictEntry)
			await DPR1_dict_xml_mod.sktXML(
				DPR_G.G_dictEntry,
				DPR_G.G_sktR[DPR_G.G_dictEntry],
			);
	...
	async function sktRootsearchstart(sectionId) {
		...
		if (getstring == "") {
			await DPR1_dict_xml_mod.sktRXML(
				DPR_translit_mod.toUni(DPR_G.G_dictEntry),
			);
			return;
		}
		...
		for (var i = 0; i < DPR_G.sktR.length; i++) {
			...
			if (yessir) {
				...
				finouta[y] =
					'<span class="pointer" style="color:' +
					DPR_G.DPR_prefs["coltext"] +
					'" onclick="DPR1_dict_xml_mod.sktRXML(' +
					i +
					');">√' +
					uniout +
					"</a><br>";
