const DPR_Dict_Xml = (() => {
	...
	function makeUniqueStringForCaseInsensitiveFS(s) {
		const charToHexStr = (c) =>
			Number(c.charCodeAt(0)).toString(16).padStart(2, "0").toUpperCase();
	...
	const createXNavButtons = (type, html, lname, nname) => {
		...
		if (lname)
			html += `<button type="button" class="btn btn-sm btn-light m-0" onclick="${onclickStrFn(lname)}" title="Previous ${type} entry..."><i class="fa fa-chevron-left"></i></button>`;
		if (nname)
			html += `<button type="button" class="btn btn-sm btn-light m-0" onclick="${onclickStrFn(nname)}" title="Next ${type} entry..."><i class="fa fa-chevron-right"></i></button>`;

		$("#bottom #difhist").html(`<div class="d-flex flex-row">${html}</div>`);
	};

	const createPEDNavToolbar = (html, lname, nname) =>
		createXNavButtons("PED", html, lname, nname);

	const createDPPNNavToolbar = (html, lname, nname) =>
		createXNavButtons("DPPN", html, lname, nname);
	...
	async function paliXML(filein, which, add) {
		...
		if (add == "right") return;
		if (add == true) {
			await DPR1_send_mod.sendPaliXML(
				DPR_translit_mod.toVel(filein.split(",")[1]),
				true,
			);
			return;
		}
		...
		if (!which) {
			...
			for (var i in DPR_G.G_pedhist) {
				if (DPR_G.G_pedhist[i] != file) {
					DPR_G.G_pedhistt.push(DPR_G.G_pedhist[i]);
				}
			}
		...
		if (DPR_G.DPR_prefs["showPedLinks"]) {
			...
			for (var i = 0; i < dataa.length; i++) {
				if (/<[^>]*$/.exec(dataa[i])) {
					// pesky broken up links
					var tda = DPR_translit_mod.toVel(
						dataa[i]
							.replace(/<[^>]*$/, "")
							.replace(/<[^>]*>/g, "")
							.replace(/ŋ/g, "ṃ")
							.toLowerCase()
							.replace(/[^āīūṭḍṅṇṃñḷĀĪŪṬḌṄṆṂÑḶa-z]/g, ""),
					);
					if (!tda || tda.length < 2) {
						datat += " " + dataa[i];
					} else if (
						typeof DPR_G.P[tda] == "object" &&
						tda != DPR_translit_mod.toVel(ttit)
					)
						datat +=
							dataa[i]
								.replace(/<[^>]*$/, "")
								.replace(
									DPR_translit_mod.toUni(tda),
									' <a style="color:' +
										DPR_G.DPR_prefs["colsel"] +
										'" href="javascript:void(0)" onclick="DPR1_dict_xml_mod.paliXML(\'PED/' +
										DPR_G.P[tda][0] +
										"," +
										DPR_translit_mod.toUni(tda) +
										"')\">" +
										DPR_translit_mod.translit(DPR_translit_mod.toUni(tda)) +
										"</a>",
								) + dataa[i].substring(dataa[i].indexOf(/<[^>]*$/));
					else datat += " " + dataa[i];
					i++;
				...
				var tda = DPR_translit_mod.toVel(
					dataa[i]
						.replace(/<[^>]*>/g, "")
						.toLowerCase()
						.replace(/[^āīūṭḍṅṇṃṃñḷĀĪŪṬḌṄṆṂÑḶa-z]/g, ""),
				);
				if (!tda || tda.length < 2) {
					datat += " " + dataa[i];
				} else if (
		...
		if (DPR_G.G_pedhist.length > 1) {
			// show select
			var showing =
				'<select title="go to history" class="mr-1" style="max-width: 10rem;" onchange="DPR1_dict_xml_mod.paliXMLHistory(this);"><option>- history -</option>';
			...
			for (var i = 0; i < DPR_G.G_peda.length; i++) {
				if (tname) {
					nname =
						DPR_G.G_peda[i][1] +
						"," +
						DPR_translit_mod.toUni(DPR_G.G_peda[i][0]);
					break;
				}
				if (
					DPR_G.G_peda[i][0] == DPR_translit_mod.toVel(ttit) &&
					DPR_G.G_peda[i][1] == DPR_G.pedfileget
				) {
					tname =
						DPR_G.G_peda[i][1] +
						"," +
		...
		if (DPR_PAL.isDictionaryFeature()) {
			...
			if (/\&entry=/.exec(permalink)) {
				permalink = permalink.replace(
					/&entry=[^&]*/,
					"&entry=" + DPR_translit_mod.toVel(filein),
				);
	...
	async function DPPNXML(filein, which, add) {
		...
		if (!which) {
			...
			for (var i in DPR_G.G_dppnhist) {
				if (DPR_G.G_dppnhist[i] != file) {
					dppnhistt.push(DPR_G.G_dppnhist[i]);
				}
			}
		...
		var data =
			...
			xmlDoc
				...
				[tloc[2]].textContent.replace(/\[/g, "<")
				.replace(/\]/g, ">")
				.replace(/href/g, 'style="color:blue" href')
				.replace(/\. {2}/g, ".&nbsp; ");
		...
		for (var i in DPR_G.G_dppn) {
			if (tname) {
				nname =
					"'" +
					DPR_translit_mod.toUni(DPR_G.G_dppn[i][0]) +
					"/" +
					DPR_G.G_dppn[i][1] +
					"','" +
					DPR_translit_mod.toUni(DPR_G.G_dppn[i][0]) +
			...
			if (
				DPR_G.G_dppn[i][0] == tloc[0] &&
				DPR_G.G_dppn[i][1] == tloc[1] + "/" + tloc[2]
			) {
				tname =
					"'" +
					DPR_translit_mod.toUni(DPR_G.G_dppn[i][0]) +
		...
		if (DPR_G.G_dppnhist.length > 1) {
			// show select
			var showing =
				'<select title="go to history" class="mr-1" style="max-width: 10rem;" onchange="DPR1_dict_xml_mod.DPPNXMLHistory(this)"><option>- history -</option>';
	...
	async function sktRXML(no, add) {
		...
		if (/[^0-9]/.test(no)) {
			...
			if (/[^0-9]/.test(no)) return;
		}

		// xml

		var xmlDoc = await DPR_DataLoader.loadSARoots(
			makeUniqueStringForCaseInsensitiveFS(DPR_G.sktR[no]),
		);
		var s = new XMLSerializer();
		var data = s.serializeToString(xmlDoc);

		data = data
			.replace(/<p><a href="index.htm">Index<\/a><\/p>/, "")
			.replace(
				/<a href="([^.]+)[^>]+/g,
				'<a class="green" href="javascript:void(0)" onclick="DPR1_dict_xml_mod.sktRXML(\'$1\')"',
			);
	...
	async function sktXML(entry, idx, which, add) {
		...
		var xmlDoc = await DPR_DataLoader.loadSADictionary(
			makeUniqueStringForCaseInsensitiveFS(char),
		);
		...
		data = data
			...
			.replace(/<(\/*)r/g, "<$1tr")
			.replace(/<(\/*)f/g, "<$1font")
			.replace(/ c=["']g/g, ' style="color:green')
			.replace(/ c=["']b/g, ' style="color:blue')
			.replace(/ c=["']r/g, ' style="color:red')
	...
	async function getAtthXML(num, type, niklist) {
		// get atthakatha or tika word
		const sectionId = DPR_Chrome.getPrimarySectionId();
		if (type == "a") {
		...
		for (var i in loca) {
			...
			var book = Number.parseInt(pca[1]) + 1;
			...
			if (u.length > 1) placen += "." + (Number.parseInt(meta) + 1);
			var v = u[meta].getElementsByTagName("h1");
			if (v.length > 1) placen += "." + (Number.parseInt(volume) + 1);
			var w = v[volume].getElementsByTagName("h2");
			if (w.length > 1) placen += "." + (Number.parseInt(vagga) + 1);
			var x = w[vagga].getElementsByTagName("h3");
			if (x.length > 1) placen += "." + (Number.parseInt(sutta) + 1);
			var y = x[sutta].getElementsByTagName("h4");
			if (y.length > 1) placen += "." + (Number.parseInt(section) + 1);
			var z = y[section]
				.getElementsByTagName("p")
				[para].textContent.substring(4);

			placen += " Para. " + (Number.parseInt(para) + 1);
			DPR_PAL.updateMostRecentUrl();

			finout +=
				'<p><span class="abut obut" onmouseup="DPR1_send_mod.openPlace(' +
				`${sectionId}, ` +
				"['" +
				nikaya +
				"'," +
				...
				type +
				"']," +
				(Number.parseInt(pca[7]) + 1) +
				",['" +
				DPR_translit_mod.toUni(word) +
				"'],'dict')\">" +
				placen +
				"</span> " +
				DPR1_format_mod.preparepali(sectionId, z, 3)[0] +
				"</p>";
		}

		displayDictData(finout);

		DPR1_chrome_mod.setCurrentTitle(
			DPR_translit_mod.toUni(word) +
				" in the " +
				DPR_G.G_hTitles[DPR_G.G_hNumbers[type]],
		);
	}

	async function getTitleXML(num, mul, att, tik, niklist) {
		// get titles for title search
		await import("./titles");
		...
		for (var i in loca) {
			...
			} else var bookno = Number.parseInt(pca[1]) - 1;
			...
			var sn =
				hiert == "m"
					...
					: null;
			const title = await DPR1_format_mod.convtitle(
				nikaya,
				book,
				una,
				vna,
			...
			var placen =
				title[0] +
				(sn ? " (" + DPR_G.G_nikLongName[nikaya] + " " + sn + ")" : "");

			finout +=
				"<p>" +
				placen +
				' <span class="abut obut" onclick="DPR1_send_mod.openPlace(' +
				`${sectionId}, ` +
				"['" +
				nikaya +
				"','" +
				...
				"','" +
				hiert +
				"'],null,null,DPR1_send_mod.eventSend(event));\">go</span></p>";
		}
		displayDictData(finout);
		if (!DPR_PAL.isXUL) {
	...
	async function getDppnData(link) {
		...
		var data =
			...
			xmlDoc
				.getElementsByTagName("e")
				[Number.parseInt(link.split("/")[1])].textContent.replace(/\[/g, "<")
				.replace(/\]/g, ">")
				.replace(/href/g, 'style="color:blue" href')
				.replace(/\. {2}/g, ".&nbsp; ");
		return data;
