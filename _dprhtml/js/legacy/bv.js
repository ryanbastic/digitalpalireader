const DPR_Bv = (() => {
	async function bv(rnd, isStatic) {
		const response = await fetch("/etc/dbv.xml");
	...
	async function showBv(rnd) {
		...
		if (!rnd) {
			...
			var today =
				'<p style="font-style:italic" name="changecolor">' +
		...
		var rd =
			'<span style="padding:2px 10px;margin-bottom:2px;" title="get random quote" onclick="DPR_bv_mod.showBv(true)" class="green small pointer">Random</span>';
		var dd = "";
		if (rnd)
			dd =
				'<span style="padding:2px 10px;" title="get today\'s quote" onclick="DPR_bv_mod.showBv()" class="green small pointer">Today</span>';

		$("#bvb, #paliquote-dialog-content").html(
			'<div style="position:absolute;top:5px;right:5px;">' +
				...
				today +
				'<p name="changecolor">' +
				abv[1].join('</p><p name="changecolor">') +
				"</p>" +
				(abv[2]
					? "<p>-- " +
						(abv[3]
							? '<span class="green pointer" onclick="DPR_bv_mod.citation(\'' +
								abv[3].replace(/^[^?]+\?/, "") +
								"',event); return false\">" +
								abv[2] +
								"</span>"
	...
	async function citation(cite, event) {
		...
		if (DPR_PAL.isWeb) {
			await DPR1_send_mod.openPlace(
				...
				param["para"],
				null,
				DPR1_send_mod.eventSend(event),
			);
		} else {
			await mainWindow.gBrowser.selectedTab.linkedBrowser.contentDocument
	...
	function bvAlert(bva) {
		...
		if (button == 0) DPR1_chrome_mod.openDPRTab(bva[3], "DPR-main", true);
	}
