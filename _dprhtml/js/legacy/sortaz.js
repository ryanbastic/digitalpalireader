const DPR_Sortaz = (() => {
	...
	function sortStrip(word) {
		...
		word = DPR_translit_mod.toUni(word.toLowerCase()).replace(
			/[^a-zāīūṃṅñṭḍṇḷ#]/g,
			"",
		);
		return word;
	...
	function findSimilarWords(word, list, min, fuzzy) {
		...
		for (var i in list) {
			...
			for (var x in word) {
				...
				if (start > -1 && start <= lstart) {
					// letter occurs before first occurrance of last letter
					lstart = start;
					var k = x;
		...
		if (count > 0) {
			simlist.sort((a, b) => Number.parseInt(b[0]) - Number.parseInt(a[0]));
			return simlist;
		}
		return null;
	}
	...
	function groupBySimilarity(list, minsim) {
		...
		for (var f = 0; f < list.length; f++) {
			if (DPR_G.devCheck)
				ddump([f + 1, "of", list.length, "words,", simlist.length, "groups"]);
			var thisword = list[f];
		...
		simlist = simlist.sort((a, b) => b.length - a.length); // by number of elements
		simlist = simlist.sort((a, b) => {
			if (b.length == a.length) return comparePaliAlphabet(a[0], b[0]);
		}); // alphabeticcaly by first element
		return simlist;
	}
