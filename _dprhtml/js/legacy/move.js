const DPR_Move = (() => {
	...
	async function moveframex(a, temp) {
		//read, etc
		if (!temp) await DPR_config_mod.getconfig();
	}

	async function moveframey(a, b) {
		//dict, conv, pad, etc - a is button, b is div (opt)
		if (!a && !b) {
			return;
		}
	...
	function scrollToId(a, b) {
		...
		document.getElementById(a).scrollTop =
			typeof b == "number" ? b : document.getElementById(b).offsetTop;
	}
