const DPR_Translit = (() => {
	...
	function toFuzzy(input) {
		...
		input = toVel(input)
			...
			.replace(/ll/g, "l")
			.replace(/ss/g, "s")
			.replace(/([kgcjtdpb])[kgcjtdpb]{0,1}h*/g, "$1");
		return input;
	}
	...
	function getTranslitLangId() {
		return TranslitCoreScriptMap[Number.parseInt(DPR_G.DPR_prefs["translits"])];
	}
	...
	function translit(data) {
		...
		const siData = DPR_translitCore_mod.convertX2SI(
			data,
			DPR_translitCore_mod.ScriptIds.RO,
		);
	...
	function convertMain(text, inScriptIndex, outScriptIndex) {
		...
		if (outScript === "VEL") {
			if (inScript !== DPR_translitCore_mod.ScriptIds.RO) {
				...
				newText = DPR_translitCore_mod.convertSI2X(
					newText,
					DPR_translitCore_mod.ScriptIds.RO,
				);
			}
