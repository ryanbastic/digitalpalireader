const DPR_Send = (() => {
	function eventSend(event, internal) {
		...
		if ((event.which == 1 || event.charCode || event.dpr_gesture) && internal)
			return "internal";
	...
	async function openPlace(
		sectionId,
		[nikaya, book, meta, volume, vagga, sutta, section, hiert, alt],
		para,
		stringra,
		add,
	) {
		...
		if (!add) {
			...
			await DPR_xml_mod.loadXMLSection(
				sectionId,
				stringra,
				para,
				[nikaya, book, meta, volume, vagga, sutta, section, hiert, alt],
		...
		if (!add) {
			...
			if (thisTab) {
				...
				await thisTabBrowser.contentDocument
					...
					.getElementsByTagName("browser")[0]
					.contentWindow.DPR_Chrome.addOrOpenMainPanelSection(
						DPR_Translations.makeDprPlace([nikaya, bookno, hier]),
						false,
						sectionId,
					);
				return;
			...
			await DPR_Chrome.addOrOpenMainPanelSection(
				DPR_Translations.makeDprPlace([nikaya, bookno, hier]),
				false,
				sectionId,
			);
			...
			if (!oldTab) {
				var permalink = DPR_PAL.toWebUrl(
					"chrome://digitalpalireader/content/index.xul?loc=" +
						nikaya +
						"." +
						bookno +
						"." +
		...
		if (!add) {
			...
			if (thisTab) {
				...
				await thisTabBrowser.contentDocument
					...
					.getElementsByTagName("browser")[0]
					.contentWindow.DPR_Chrome.addOrOpenMainPanelSection(
						DPR_Translations.makeDprPlace([nikaya, bookno, DPR_G.G_hier]),
						false,
						sectionId,
					);
				return;
	...
	async function sendPaliXML(link, add) {
		if (!add) {
			...
			var thisTab = DPR1_chrome_mod.isDPRTab("DPRm");
			if (thisTab) {
				await DPR_PAL.mainWindow.gBrowser
					.getBrowserForTab(thisTab)
					.contentWindow.DPR1_dict_xml_mod.paliXML(link);
				return;
			}
			var oldTab = DPR1_chrome_mod.findDPRTab("DPR-main");
			...
			if (!oldTab) {
				var permalink = DPR_PAL.toWebUrl(
					"chrome://digitalpalireader/content/dict.htm?type=PED&opts=xv,xd,xm,xs,xa,xk,xy,mm,ma,mt,sw,hd&query=" +
						link,
				);
	...
	async function sendDPPNXML(link, add) {
		if (!add) {
			...
			if (thisTab) {
				await DPR_PAL.mainWindow.gBrowser
					.getBrowserForTab(thisTab)
					.contentWindow.DPR1_dict_xml_mod.DPPNXML(link);
				return;
			}
			var oldTab = DPR1_chrome_mod.findDPRTab("DPR-main");
			...
			if (!oldTab) {
				var permalink = DPR_PAL.toWebUrl(
					"chrome://digitalpalireader/content/dict.htm?type=DPPN&opts=xv,xd,xm,xs,xa,xk,xy,mm,ma,mt,sw,hd&query=" +
						DPR_translit_mod.toVel(link),
				);
				DPR1_chrome_mod.openDPRTab(permalink, "DPRd");
			} else {
				DPR_PAL.mainWindow.gBrowser.selectedTab = oldTab;
				await DPR_PAL.mainWindow.gBrowser
					.getBrowserForTab(oldTab)
					.contentWindow.DPR1_dict_xml_mod.DPPNXML(link);
			}
		} else if (add != "right") {
			var permalink = DPR_PAL.toWebUrl(
	...
	async function sendAtt(x, type, nik, add) {
		if (!add) {
			...
			if (!oldTab) {
				...
				await DPR_PAL.mainWindow.gBrowser
					.getBrowserForTab(oldTab)
					.contentDocument.getElementById("dpr-tops")
					.getElementsByTagName("browser")[0]
					.contentWindow.DPR1_dict_xml_mod.getAtthXML(x, type, nik);
			}
		} else if (add != "right") {
			var permalink = DPR_PAL.toWebUrl(
				"chrome://digitalpalireader/content/index.xul" +
		...
		if (!add) {
			...
			if (!oldTab) {
				...
				await oldTabBrowser.contentDocument
					.getElementById("dpr-tops")
					.getElementsByTagName("browser")[0]
					.contentWindow.DPR1_dict_xml_mod.getTitleXML(x, m, a, t, nik);
			}
		} else if (add != "right") {
			var permalink = DPR_PAL.toWebUrl(
				"chrome://digitalpalireader/content/index.xul" +
	...
	async function sendUpdateBookmarks() {
		...
		for (var i = 0; i < oldTabs.length; i++) {
			var oldTabBrowser = DPR_PAL.mainWindow.gBrowser.getBrowserForTab(
				oldTabs[i],
			);
	...
	async function sendAnalysisToOutput(
		sectionId,
		input,
		divclicked,
		frombox,
		...
		if (divclicked) {
			...
			if (cdiv) {
				...
				if (ldiv) {
					...
					if (/varc/.test(lcn)) ldiv.style.color = DPR_G.DPR_prefs["grey"];
					else ldiv.style.color = DPR_G.DPR_prefs["coltext"];
			...
			if (DPR_G.DPR_prefs["copyWord"]) DPR_PAL.copyToClipboard(input);
		}
		if (add != true) {
			// reuse old tab
			var thisTab = DPR1_chrome_mod.isDPRTab("DPRm");
			...
			var permalink = DPR_PAL.toWebUrl(
				"chrome://digitalpalireader/content/bottom.htm" +
					"?analysis=" +
					DPR_translit_mod.toVel(input) +
					"&frombox=" +
					frombox,
	...
	function sendTranslate(input, add) {
		...
		if (add != true) {
			...
			var permalink = DPR_PAL.toWebUrl(
				"chrome://digitalpalireader/content/translate.htm" +
					"?phrase=" +
					DPR_translit_mod.toVel(input),
			);
			DPR1_chrome_mod.openDPRTab(permalink, "DPRx");
	...
	async function openTranslation(url, add) {
		...
		if (add == "shift") {
			DPR1_chrome_mod.openDPRTab(
				`${DPR_Translations.resolveUri(sInfo)}`,
				"DPRx",
			);
		} else {
			var thisTab = DPR1_chrome_mod.isDPRTab("DPRm");
	...
	function closePanel() {
		var cW =
			DPR_PAL.mainWindow.gBrowser.selectedTab.linkedBrowser.contentWindow;
		var cD =
			DPR_PAL.mainWindow.gBrowser.selectedTab.linkedBrowser.contentDocument;
		...
		window.addEventListener("unload", () => {
			cW.DPR1_send_mod.reindexPanels();
		});
		...
		url[1] = panels;
		var newurl =
			url.join("?").replace(/\?$/, "") + (bottom ? "&" + bottom.join("&") : "");
		cW.history.replaceState({}, "Title", newurl);

		elem.removeChild(
			elem.getElementsByTagName("splitter")[DPR_G.G_compare - 1],
		);
