const DPR_Translate = (() => {
	function translateTextx() {
		...
		for (var i in DPR_G.yt) {
			window.dump(
				cnt +
					" of 20926 - " +
					Math.floor((cnt++ / 20926) * 100) +
					"% finished\n",
			);
			var aa = [],
				bb = [];
			var a = new RegExp(
				"(^|[ .,;])" + DPR_translit_mod.toUni(i) + "($|[ .,;])",
			);
			for (var j in epd) {
				b = epd[j].split("^");
				if (a.test(b[1])) {
		...
		for (var i in cc) x += 'eg["' + i + '"] = "' + cc[i] + '";<br/>';
		for (var i in dd) y += 'eg["' + i + '"] = "' + dd[i] + '";<br/>';
	...
	async function translateText(alts) {
		if (/^zzz/.test($("#input").val())) return translateTextx();
		var words = [];
		var subject = "";
		var outparts = [];
		...
		if (!out.subject.length) {
			if (DPR_G.G_verbDecl.length)
				subject =
					DPR_G.G_subjects[DPR_G.G_verbDecl[1] - 1][DPR_G.G_verbDecl[2] - 1];
		} else {
		...
		$("#translation").html(
			DPR1_format_mod.makeTable(
				[outparts.concat([DPR_G.G_verbDecl[0] == 2 ? "!" : "."])],
				"trans",
			),
		);
	}
  ...
  */
		...
		for (var i = 0; i < words.length; i++) {
			for (var j = 0; j < words[i].length; j++) {
				if (alts && alts[i] && alts[i][1] != 0 && alts[i][1] != j)
					// chose another alternative
					continue;
				if (!words[i][j][1] && !words[i][j][2]) tint = "o";
				else if (words[i][j][1] == "bv") tint = "bv";
				else if (words[i][j][1] == "v") tint = "v";
				else if (words[i][j][2]) tint = words[i][j][2][1] - 1;
				else tint = "o";

				if (words[i].length == 1) {
					// force place if necessary
					if (tint == "v") DPR_G.G_verbDecl = words[i][j][2];
					else if (tint == 0) DPR_G.G_subDecl = words[i][j][2];
					else if (tint == 1) DPR_G.G_objDecl = words[i][j][2];

					chosen[i] = [tint, j];
				}
				if (!chosen[i]) inter[tint].push([i, j]);
			}
		}
		...
		if (inter[1].length) {
			...
			for (var i in inter[1])
				if (!chosen[inter[1][i][0]]) choices1.push(inter[1][i]);
		...
		if (inter["v"].length) {
			...
			for (var i in inter["v"])
				if (!chosen[inter["v"][i][0]]) choicesV.push(inter["v"][i]);
		...
		if (inter[0].length && DPR_G.G_verbDecl) {
			...
			for (var i in inter[0]) {
				if (!chosen[inter[0][i][0]]) choices0.push(inter[0][i]);
			}
			if (choices0.length) {
				...
				if (compat[1]) {
					...
					DPR_G.G_subDecl =
						words[compat[0][0][0]][chosen[compat[0][0][0]][1]][2];
					inter[0] = [];
				}
			}
		}
		...
		for (var i in words) {
			if (chosen[i]) {
				if (!alts)
					// don't recreate if we are using it
					DPR_G.G_altChoices[i] = [words[i], j];
				if (words[i][chosen[i][1]][2] && words[i][chosen[i][1]][2][1] == 6) {
					// genitive, keep with next
					pendG.push(words[i][chosen[i][1]]);
					continue;
				}
				if (words[i][chosen[i][1]][1] == "i") {
					// indec, keep with next
			...
			for (var j in words[i]) {
				// for the rest, choose first, then alt the rest
				...
				if (!w[1] && !w[2]) tint = "o";
				else if (w[2]) tint = w[2][1] - 1;
				else tint = "o";
				if (pendI.length) {
					// add indeclinables
					for (var k in pendI) outer[tint].push(pendI[k]);
					pendI = [];
				}
				if (pendG.length) {
					// add genitives
					for (var k in pendG) outer[tint].push(pendG[k]);
					pendG = [];
				}
				outer[tint].push(w);
				break;
		...
		for (var i in outer) {
			if (
				outer[i].length > 1 ||
				(outer[i].length == 1 && outer[i][0].length > 1)
			) {
				var joined = "";
				if (i != "v" && i != "o")
					// nominal, add prepositions, plural
					joined = addPhrasePreps(outer[i], i, "n", true);
				else if (i == "v")
					// verbal, add prepositions
				...
				if (i == 0) out.subject.push(joined);
				else if (i == 1) out.object.push(joined);
				else if (i == "bv") out.bverb.push(joined);
				else if (i == "v") out.verb.push(joined);
				else out.other.push(joined);
			}
		}
		return out;
	}
	...
	function checkCompatibleNoun(input, chosen, words) {
		...
		if (input.length || !outerb.length) return [input, false];
		else return [outerb, true];
	}
	...
	function checkCompatibleVerb(input, chosen, words) {
		...
		if (input.length) return [input, false];
		else return [outerb, true];
	}
	...
	function makeWord(word, pl, alts) {
		return (
			...
			(alts
				? ' onmouseover="DPR_translate_mod.showAltTable(' + word[5] + ')"'
				: "") +
			' target="_blank" href=' +
			DPR_PAL.dprHomePage +
			"?analysis=" +
			DPR_translit_mod.toVel(word[3]) +
			'" title="' +
			(word[0] == word[3] ? "lookup " : "translation of ") +
			word[3] +
			'">' +
			(pl ? addPlural(word[0]) : word[0]) +
			"</a>"
		);
	}
	...
	async function translateWord(word, idx) {
		...
		if (eg[vword]) {
			...
			if (DPR_G.yt[vword]) {
				...
				for (var p in DPR_G.P[temp]) {
					...
					if (/adj\./.test(type)) type = "p";
					else if (/indecl\./.test(type)) type = "i";
					else if (/(adj|nt|m|f)\./.test(type)) type = "n";
					else if (/^to /.test(trans)) type = "v";
					else type = "n";
			...
			for (var a in wtr) {
				...
				if (DPR_G.yt[temp] && DPR_G.yt[temp][4] != "I") {
					...
					for (var c in declt) {
						if (
							type == "v" ||
							/adj\./.test(DPR_G.yt[temp][1]) ||
							(1 & declt[c][0] && DPR_G.G_nTx[0].test(gender)) ||
							(2 & declt[c][0] && DPR_G.G_nTx[1].test(gender)) ||
							(4 & declt[c][0] && DPR_G.G_nTx[2].test(gender))
						) {
							decls.push(declt[c]);
						}
					}
					...
					if (
						DPR_G.yt[DPR_G.G_irregDec[temp][0]][4] == "P" ||
						DPR_G.yt[DPR_G.G_irregDec[temp][0]][1] == "adj."
					)
						type = "p";
					for (var c in decls) {
					...
					for (var p in DPR_G.P[temp]) {
						...
						} else type = DPR_G.G_endings[wtr[a][1]][4];
		...
		var dups = [];
		var outfin = [];
		dupsl: for (var i = 0; i < outs.length; i++) {
			...
			for (var j = 0; j < dups.length; j++) {
				if (
					...
					((!dups[j][2] && !outs[i][2]) ||
						(dups[j][2] &&
							outs[i][2] &&
							dups[j][2][0] == outs[i][2][0] &&
							dups[j][2][1] == outs[i][2][1] &&
							dups[j][2][2] == outs[i][2][2]))
				) {
					continue dupsl;
				}
			}
	...
	function transMod([trans, type, deca, word, meta]) {
		if (type == "n") trans = trans.replace(/^(an*|the) /, "");
		if (type == "n" && deca && deca[1] == 1)
			// noun subject
			DPR_G.G_subDecl = deca;
		if (type == "v" && deca) {
			DPR_G.G_verbDecl = deca;
			if (!(1 & deca[0])) trans = trans.replace(/\bis\b/, "be");
			else {
				trans = trans.replace(/\bbe\b/, "is");
				trans = trans.replace(/\bis\b/, DPR_G.G_ises[deca[1] - 1][deca[2] - 1]);
			}
			if (!(deca[0] & 1) || deca[1] + deca[2] != 2) {
				// verb endings, not present or not 3rd sing
	...
	function addPlural(word, type) {
		if (/\bone\b/.test(word)) return word.replace(/\bone\b/, "ones");
		else if (/man\b/.test(word)) word = word.replace(/man\b/, "men");
		else if (/(\S\S+)y$/.test(word) && type != "v")
			word = word.replace(/y$/, "ies");
		else if (/(\S\S+)f$/.test(word)) word = word.replace(/f$/, "ves");
		else if (/(\S+)(s|[sc]h|zz|[xo])$/.test(word)) word += "es";
		else word += "s";
		return word;
	}
	...
	function addPhrasePreps(words, i, type, alts) {
		...
		for (var j = 0; j < words.length; j++) {
			if (typeof words[j] == "string") {
				...
				if (typeof type == "undefined") type = words[j][1];
				if (type == "p") type = "n";
				...
				if (j == 0) {
					if (DPR_G.G_joints[type] == null) return words[j][3];
					joined += DPR_G.G_joints[type][i]
						? DPR_G.G_joints[type][i] + " "
						: "";
				}
				if (j > 0) joined += " ";
				if (words[j + 1]) joined += makeWord(words[j], false, alts);
				// last word, check plural
				else
					joined += makeWord(
						words[j],
						type == "n" && words[j][2] && words[j][2][2] == 2,
						alts,
					);
			}
		}
	...
	function sortLongerDec(a, b) {
		if (DPR_G.G_endings[a[1]][4] == "v") return -1;
		if (DPR_G.G_endings[b[1]][4] == "v") return 1;
	...
	function showAltTable(idx) {
		...
		var out = "<b>" + w[0][3] + "</b> ";
		if (w.length == 1) out += DPR_grammar_mod.makeGrammarTerms(w[0]);
		else if (w.length > 1) {
			out += '<select onclick="DPR_translate_mod.changeAlt(this,' + idx + ')">';
			for (var i in w) {
				out +=
					"<option>" + DPR_grammar_mod.makeGrammarTerms(w[i]) + "</option>";
			}
			out += "</select>";
		} else return;
	...
	async function insertWordByWord() {
		var input = DPR_translit_mod.toUni($("#input").val().toLowerCase())
			.replace(/(\n|\r)/g, " ")
			.replace(DPR_G.G_uniRegExpNSG, "");
		...
		for (var i = 0; i < words.length; i++) {
			...
			for (var j = 0; j < words[i].length; j++) {
				options +=
					'<li><span class="decl-grammar">' +
					words[i][j][0] +
					" form of " +
					words[i][j][1] +
					'</span><br/><span class="decl-english">' +
					words[i][j][2] +
					"</span></li>";
				options =
					"$('#').html(unescape('" + escape("<ol>" + options + "</ol>") + "'))";
			}
