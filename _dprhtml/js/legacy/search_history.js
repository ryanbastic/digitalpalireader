const DPR_Search_History = (() => {
	...
	async function simSearchHistory(data) {
		...
		if (dataJsObj.displayText.localeCompare("-- History --") !== 0) {
			const searchType = dataJsObj.searchType,
				searchString = dataJsObj.query,
				searchMAT = dataJsObj.MAT,
				searchSet = dataJsObj.sets,
				searchBook = dataJsObj.book,
				searchPart = dataJsObj.part,
				searchRX = dataJsObj.rx;
			...
			if (searchType == 0 || searchType == 2) {
				for (var i in DPR_G.G_hLetters) {
					if (searchMAT.search(DPR_G.G_hLetters[i]) > -1)
						document.getElementById("tsoMAT" + DPR_G.G_hLetters[i]).checked =
							true;
					else
						document.getElementById("tsoMAT" + DPR_G.G_hLetters[i]).checked =
							false;
				...
				document.getElementById("tsoMAT2m").selectedIndex =
					DPR_G.G_hNumbers[searchMAT];
			...
			if (searchType == 0 || searchType == 5) {
				for (var i in DPR_G.G_nikToNumber) {
					...
					else document.getElementById("tsoCO" + i).checked = false;
				}
			} else {
			...
			if (searchType == 1) {
				for (
					var i = 0;
					i < document.getElementById("tsoBOOKm").itemCount;
					i++
				) {
					if (searchBook.search(i + 1) > -1)
						document.getElementById("tsoBObook" + i).checked = true;
					else document.getElementById("tsoBObook" + i).checked = false;
				}
			} else {
			...
			if (searchType == 3) {
				...
				for (var i = 1; i < parts.length; i++) {
					document.getElementById(
						"tsoP" + DPR_G.G_listTitles[i - 1],
					).selectedIndex = parts[i];
					await DPRXML.updateSearchHierarchy(i);
				}
				document.getElementById("tsoPR").selectedIndex = parts[0];
				DPROpts.chooseSearchHier(Number.parseInt(parts[0]) + 1);
			}
	...
	function addSearchHistory(query, searchType, rx, sets, MAT, book, part) {
		...
		searchHistStoreObj.displayText = getSearchHistDisplayText(
			query,
			searchType,
			rx,
			sets,
		...
		const searchHistoryArrayFromStorage =
			localStorage.getItem("searchHistoryArray");
	...
	function getSearchHistDisplayText(
		query,
		searchType,
		rx,
		sets,
		...
		const types = ["Sets", "Books", "Book", "Part", "", "ATI"];
		let place = "";
