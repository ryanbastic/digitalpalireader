const DPR_Search = (() => {
	...
	async function searchTipitaka(
		...
		if (searchString) {
			...
			var permalink = DPR_PAL.toWebUrl(
				"chrome://digitalpalireader/content/search.xul" +
					"?type=" +
					searchType +
					"&query=" +
					searchString +
			...
			var options = DPR_G.MD.location.href
				.split("?")[1]
				.split("#")[0]
				.split("&");
			...
			for (var i in options) {
				...
				switch (option[0]) {
					case "type":
						DPR_G.G_searchType = Number.parseInt(option[1]);
						break;
					case "query":
		...
		if (!DPR_G.G_searchString) {
			...
			var input = { value: "" };
			var result = DPR_G.G_prompts.prompt(
				null,
				"Enter Query",
				"Enter query for custom search:",
				input,
				null,
				check,
			);
		...
		st[2] =
			DPR_G.G_nikLongName[DPR_G.G_searchSet] +
			" " +
			DPR_G.G_searchBook.slice(1, -1);
		st[3] =
			DPR_G.G_nikLongName[DPR_G.G_searchSet] +
			" " +
			DPR_G.G_searchBook.slice(1, -1) +
		...
		if (DPR_G.G_searchType == 0 && /\|/.test(DPR_G.G_searchBook))
			// custom search
			st[0] += " (custom)";
		...
		var tabT =
			"Search: '" +
			(DPR_G.G_searchRX
				? DPR_translit_mod.toUniRegEx(DPR_G.G_searchString)
				: DPR_translit_mod.toUni(DPR_G.G_searchString)) +
			"' in " +
			st[DPR_G.G_searchType];

		DPR_search_mod.setTitle(tabT);

		if (/^[TPVMtpvm][0-9]\.[0-9][0-9][0-9][0-9]$/.exec(DPR_G.G_searchString)) {
			// page search
	...
	function DPR_PAL_Search_ScrollSearch(what) {
		if (DPR_PAL.isXUL) {
			document.getElementById("search").scrollTop = what
				? document.getElementById(what).offsetTop
				: 0;
		} else {
			const searchHitsSectionHeight =
				document.getElementById("sbfab").offsetHeight;
			const scrollBy = what
				? document.getElementById(what).offsetTop + searchHitsSectionHeight
				: 0;
	...
	function finishSearch() {
		...
		const searchLink =
			"dpr:index?feature=search&type=" +
			DPR_G.G_searchType +
			"&query=" +
			DPR_translit_mod.toVel(DPR_G.G_searchString) +
			"&MAT=" +
			DPR_G.G_searchMAT +
			"&set=" +
			DPR_G.G_searchSet +
	...
	async function pausesall() {
		...
		for (var w in DPR_G.G_XMLFileArray) {
			...
			for (var x = 0; x < 3; x++) {
				if (
					DPR_G.G_searchMAT.indexOf(DPR_G.G_hLetters[x]) > -1 &&
					DPR_G.G_XMLFileArray[w][x] == 1
				) {
					// this hier is checked and the file exists in this hier
					if (
						cbooks &&
						cbooks[cniks[w.charAt(0)]] &&
						cbooks[cniks[w.charAt(0)]].indexOf("," + w.substring(1) + ",") == -1
					)
						continue; // skip unspecified books for custom search
		...
		DPR_search_mod.addSearchTermSectionLink(
			DPR_G.G_searchRX
				? DPR_G.G_searchString
				: DPR_translit_mod.toUni(DPR_G.G_searchString),
		);
	...
	async function pausetwo() {
		// init function for single collection
		...
		for (var w in DPR_G.G_XMLFileArray) {
			...
			if (DPR_G.G_searchBook.indexOf("," + w.substring(1) + ",") == -1)
				continue; // skip unchecked books
	...
	async function pausethree() {
		...
		var getstring = DPR_G.G_searchString;
		if (which == 2) {
			// single book, multiple hier
			for (var x = 0; x < 3; x++) {
				if (
					DPR_G.G_searchMAT.indexOf(DPR_G.G_hLetters[x]) > -1 &&
					DPR_G.G_XMLFileArray[nikbook][x] == 1
				) {
					// this hier is checked and the file exists in this hier
		...
		DPR_search_mod.addSearchTermSectionInfo(
			DPR_G.G_nikLongName[nikaya] +
				" " +
				book +
				(DPR_G.G_searchPart != "1" ? " (partial)" : ""),
		);
	...
	async function importXMLs(cnt) {
		...
		if (cnt == 1) {
			// whole tipitaka or multiple collections
			...
			if (DPR_G.qz < DPR_G.G_searchFileArray.length - 1) {
				...
				if (DPR_G.nextbookfile.charAt(0) != DPR_G.nikayaat)
	...
	function createTables(xmlDoc, hiert) {
		...
		var bookname = DPR_navigation_mod.getBookName(
			nikaya,
			hiert,
			Number.parseInt(book) - 1,
		);
		...
		if (yesplus >= 0) {
			stringra = (
				DPR_G.G_searchRX
					? DPR_translit_mod.toUniRegEx(getstring)
					: DPR_translit_mod.toUni(getstring)
			).split("+");
		} else {
		...
		for (
			...
			if (
				DPR_G.G_searchType == 3 &&
				part[0] >= 0 &&
				sx != Number.parseInt(part[1])
			)
				continue;
					...
					for (
						...
						for (
							...
							for (
								...
								if (!/[0-9]/.exec(DPR_G.G_searchString))
									texttomatch = texttomatch.replace(/\^a\^[^^]*\^ea\^/g, ""); // remove pesky page references unless we're searching for them.
								...
								if (yesplus >= 0) {
									...
									for (var d = 0; d < stringra.length; d++) {
										...
										if (startmatch >= 0) {
											...
											while (startmatch >= 0) {
												if (DPR_G.G_searchRX)
													gotstring = texttomatch.match(
														getRegExtSearchString(perstring),
													)[0];
												...
												if (
													gotstring.indexOf("<c") == -1 &&
													gotstring.indexOf("c>") == -1
												) {
													// make sure we're not doubling
													...
													postpara +=
														...
														gotstring
															.replace(/^ /g, "")
															.replace(/ $/g, "")
															.replace(/(.) (.)/g, "$1<xc> <c" + d + ">$2") +
														"<xc>" +
														(gotstring.charAt(gotstring.length - 1) == " "
															? " "
															: "");
													...
													aftermex =
														spacea == -1 ? afterm : afterm.substring(0, spacea);
													tempexword[d].push(
														(beforem + gotstring + aftermex)
															.replace(/[‘’“”]/g, "")
															.replace(/<[^>]*>/g, ""),
													);
									...
									if (yesall == stringra.length) {
										for (var t = 0; t < tempexword.length; t++) {
											...
											for (var i = 0; i < l; i++) {
												if (
													/^[TPVM][0-9]\.[0-9][0-9][0-9][0-9]$/.exec(getstring)
												) {
													// page search
													...
													while (
														!/[0-9]/.exec(
															tempexword[t][i].charAt(
																tempexword[t][i].length - 1,
															),
														)
													) {
													...
													tempexword[t][i] = tempexword[t][i].replace(
														/\^e*b\^/g,
														"",
													);
													while (
														!DPR_G.G_uniRegExp.exec(
															tempexword[t][i].charAt(0),
														) &&
														!/[0-9]/.exec(tempexword[t][i].charAt(0))
													) {
														tempexword[t][i] = tempexword[t][i].substring(1);
													}
													while (
										...
										for (var t = 0; t < tempexword.length; t++) {
											...
											for (var i = 0; i < l; i++) {
												for (var j = i + 1; j < l; j++) {
													if (tempexword[t][i] === tempexword[t][j]) j = ++i;
												}
												texnodups[t].push(tempexword[t][i]);
											}
											tagtitle += (
												"q" +
												texnodups[t].join("q").replace(/\"/g, "x") +
												"q"
											).replace(/ +/g, "_");
										}
										...
										if (u.length > 1) {
											finalout +=
												', <b style="color:' +
												DPR_G.DPR_prefs[cola[colt]] +
												'">' +
												DPR_translit_mod.toUni(
													u[sx]
										...
										finalout +=
											"</span>, para. " +
											(tmp + 1) +
											' <span class="abut obut" onmouseup="DPR1_send_mod.openPlace(' +
											`${sectionId}, ` +
											"['" +
											nikaya +
											"'," +
											...
											",'" +
											sraout +
											"',DPR1_send_mod.eventSend(event))\">&rArr;</span></span></p><p>" +
											DPR1_format_mod.preparepali(sectionId, postpara, 1)[0] +
											"</p><hr></div>";
									...
									const searchPattern = DPR_G.G_searchRX
										? getRegExtSearchString(getstring)
										: getstring;
									let searchResult = searchAcrossBoldBoundary(
										texttomatch,
										searchPattern,
									);
									...
									if (startmatch >= 0) {
										while (startmatch >= 0) {
											...
											postpara +=
												...
												gotstring
													...
													.replace(/ $/g, "")
													.replace(/(.) (.)/g, "$1<xc> <c0>$2")
													.replace(new RegExp(/(\^e?b\^)/, "g"), "<xc>$1<c0>") +
												"<xc>" +
												(gotstring.charAt(gotstring.length - 1) == " "
													? " "
													: "");
											...
											searchResult = searchAcrossBoldBoundary(
												texttomatch,
												searchPattern,
											);
											startmatch =
												searchResult === null ? -1 : searchResult.start;
											...
											tempexword.push(
												(beforem + gotstring + aftermex).replace(/[‘’“”]/g, ""),
											);
										}
										...
										if (/^[TPVM][0-9]\.[0-9][0-9][0-9][0-9]$/.exec(getstring)) {
											// not page search
											for (var i = 0; i < l; i++) {
												...
												while (
													!/[0-9]/.exec(
														tempexword[i].charAt(tempexword[i].length - 1),
													)
												) {
											...
											for (var i = 0; i < l; i++) {
												...
												while (
													!DPR_G.G_uniRegExp.exec(tempexword[i].charAt(0)) &&
													!/[0-9]/.exec(tempexword[i].charAt(0))
												) {
													tempexword[i] = tempexword[i].substring(1);
												}
												while (
										...
										finalout +=
											...
											",'" +
											sraout +
											"','search')\">&rArr;</span></span></p><p>" +
											DPR1_format_mod.preparepali(sectionId, postpara, 3)[0] +
											"</p><hr></div>";
	...
	function showonly(string) {
		...
		if (string == "xyz") {
			...
			for (var x = 0; x < da.length; x++) {
				if (
					(da[x].id.indexOf("q" + string + "q") > -1 || !da[x].id) &&
					da[x].id != "xyz"
				)
					da[x].style.display = "block";
				else da[x].style.display = "none";
			}
			const linkText = normalizeLongSearchResult(
				DPR_translit_mod.toUni(string.replace(/xn/g, '"n').replace(/_/g, " ")),
			);
			$("#showing").html(
				'<b style="color:' +
					DPR_G.DPR_prefs["colped"] +
					'">' +
					DPR_translit_mod.translit(linkText) +
					"&nbsp;</b><b>x</b>",
			);
	...
	async function atiSearchStart() {
		...
		var atiurl = DPR_G.DPR_prefs["catioff"]
			? "file://" + DPR_G.DPR_prefs["catiloc"].replace(/\\/g, "/") + "/html/"
		...
		if (DPR_G.DPR_prefs["catioff"]) {
			...
			for (var i = 0; i < DPR_G.G_numberToNik.length; i++) {
				...
				thisset.setAttribute(
					"onmouseup",
					"DPR1_search_mod.scrollSearch('sbfN" + DPR_G.G_numberToNik[i] + "')",
				);

				var setlabel = DPR_G.MD.createElement("label");
				setlabel.setAttribute(
					"value",
					DPR_G.G_nikLongName[DPR_G.G_numberToNik[i]] + ": 0",
				);
			...
			$("#stfc").html("");
			$("#sbfab").html(
				'<div id="dictList"><p class="huge">Matched Suttas:</p></div><hr class="thick">',
			);
		...
		bannerDiv.innerHTML =
			...
			atiurl +
			'favicon.ico" title="Search courtesy of http://www.accesstoinsight.org/" onclick="window.open(\'http://www.accesstoinsight.org/\')">&nbsp;<a href="http://www.accesstoinsight.org/" target="_blank" style="color:blue">AccessToInsight.org</a> search for <b>' +
			getstring +
			"</b>:<br><br>";
		...
		outNode.setAttribute(
			"height",
			document.getElementById("sbfbc").offsetHeight,
		);
		outNode.setAttribute(
			"src",
			"http://www.google.com/cse?cx=015061908441090246348%3Aal1bklhbjbi&cof=FORID%3A9%3BNB%3A1&ie=UTF-8&q=" +
				getstring +
				"+more:suttas_only&sa=Search&ad=w9&num=10",
		);
		$("#sbfb").html("");
		document.getElementById("sbfb").appendChild(outNode);
	}
	...
	function atiSearchOffline(d, getstring) {
		...
		for (var c = 0; c < anik.length; c++) {
			...
			var title = DPR_translit_mod.toUni(
			...
			for (var j in data) {
				if (data[j].id == "H_content") {
					...
					if (startmatch >= 0) {
						listout +=
							"<a href=\"javascript:void(0)\" onclick=\"DPR_move_mod.scrollToId('search','atio" +
							nik +
							c +
							'\')" style="color:' +
							DPR_G.DPR_prefs["colsel"] +
							'" title="show results in sutta">' +
							title +
							'</a>&nbsp;<a class="small green" href="file://' +
							DPR_G.DPR_prefs["catiloc"].replace(/\\/g, "/") +
							"/html/tipitaka/" +
							anik[c] +
							'" target="_blank" title="open sutta">&rArr;</a><br/>';
						while (startmatch >= 0) {
							count++;
							...
							while (startmatch - buffers > 0) {
								// get first space before buffer
								if (texttomatch.charAt(startmatch - buffers - 1) == " ") break;
								buffers--;
							}
							while (endmatch + buffere < texttomatch.length) {
								// get first space before buffer
							...
							beforem =
								"... " +
								texttomatch.substring(startmatch - buffers, startmatch);
							afterm =
								texttomatch.substring(endmatch, endmatch + buffere) + " ...";
							postpara +=
								beforem +
								"<c0>" +
						...
						postpara = postpara
							.replace(
								/<c0>/g,
								'<span style="color:' + DPR_G.DPR_prefs["colped"] + '">',
							)
							.replace(/<xc>/g, "</span>");
						finalout +=
							"<div id=atio" +
							nik +
							c +
							'><p><br><b><a class="green" href="file://' +
							DPR_G.DPR_prefs["catiloc"].replace(/\\/g, "/") +
							"/html/tipitaka/" +
							anik[c] +
							'" target="_blank" title="open sutta">' +
							title +
							'</a></b> <a href="javascript:void(0)" onclick="DPR_move_mod.scrollToId(\'search\',0);" class="small" style="color:' +
							DPR_G.DPR_prefs["coldppn"] +
							'">top</a></p><p>' +
							postpara +
							"</p></div>";
					}
		...
		cellNode.innerHTML =
			...
			(count > 0
				? " onclick=\"DPR_move_mod.scrollToId('dif','atiL" + nik + "')\""
				: "") +
			">" +
			DPR_G.G_nikLongName[nik] +
			"</a>: " +
			count +
			"; ";

		var val = DPR_G.MD.getElementById("matches" + nikA[d])
			.getAttribute("value")
			.replace(/: .+/, ": ");
		DPR_G.MD.getElementById("matches" + nikA[d]).setAttribute(
			"value",
			val + count,
		);
	...
	function getRegExtSearchString(string) {
		return new RegExp(
			DPR_translit_mod.toUniRegEx(string).replace(
				/\\b/g,
				"([^AIUEOKGCJTDNPBMYRLVSHaiueokgcjtdnpbmyrlvshāīūṭḍṅṇṁṃñḷĀĪŪṬḌṄṆṀṂÑḶ]|^|$)",
			),
		);
	}
