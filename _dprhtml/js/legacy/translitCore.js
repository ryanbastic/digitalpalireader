const DPR_TranslitCore = (() => {
	...
	const paliScriptInfo = new Map([
		...
		[
			Script.HI,
			["Devanagari", "देवनागरी", [[0x0900, 0x097f]], { f: "in_flag.png" }],
		],
		[
			Script.RO,
			[
		...
		[
			Script.THAI,
			["Thai", "ไทย", [[0x0e00, 0x0e7f], 0xf70f, 0xf700], { f: "th_flag.png" }],
		...
		[
			Script.KM,
			["Khmer", "ភាសាខ្មែរ", [[0x1780, 0x17ff]], { f: "kh_flag.png" }],
		],
		[
			Script.BENG,
			[
		...
		[
			...
			[
				...
				"བོད་སྐད།",
				[[0x0f00, 0x0fff]],
				{ f: "tibet_flag.png", c: "larger", g: "other" },
			],
		],
		[
			Script.CYRL,
  ...
  ['\u200D', ''], // ZWJ (remove) will be added when displaying*/
	...
	function beautify_tibet(text) {
		...
		for (let i = 0; i <= 39; i++) {
			text = text.replace(
				new RegExp(String.fromCharCode(0x0f84, 0x0f40 + i), "g"),
				String.fromCharCode(0x0f90 + i),
			);
		}
	...
	function convert_to(text, script) {
		const hashMaps = prepareHashMaps(
			script_index[Script.SI],
			script_index[script],
		);
	...
	class TextProcessor {
		...
		static basicConvert(text, script) {
			(convert_to_func[script] || convert_to_func_default).forEach(
				(func) => (text = func(text, script)),
			);
			return text;
		}
		// convert from another script to sinhala
		static basicConvertFrom(text, script) {
			(convert_from_func[script] || convert_from_func_default).forEach(
				(func) => (text = func(text, script)),
			);
			return text;
		}
		...
		static beautify(text, script, rendType = "") {
			(beautify_func[script] || beautify_func_default).forEach(
				(func) => (text = func(text, script, rendType)),
			);
			return text;
		}
		...
		static convertFrom(text, script) {
			(un_beautify_func[script] || un_beautify_func_default).forEach(
				(func) => (text = func(text, script)),
			);
