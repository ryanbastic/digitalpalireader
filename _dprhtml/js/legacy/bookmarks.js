const DPR_Bookmarks = (() => {
	async function eraseBookmark(i) {
		if (window.DPR_Globals.NavigationTabViewModel.isStorageSupportedByBrowser) {
			const bookmarksArrayFromStorage = localStorage.getItem("bookmarksArray");
			let data = [];
	...
	async function clearBookmarks(gofrom) {
		if (window.DPR_Globals.NavigationTabViewModel.isStorageSupportedByBrowser) {
			var answer = confirm(
				"Are you sure you want to erase all of the stored bookmarks?",
			);
			if (!answer) {
				return;
			}
	...
	function getBookmarks() {
		if (window.DPR_Globals.NavigationTabViewModel.isStorageSupportedByBrowser) {
			...
			const content = [];
			if (bookmarksArrayFromStorage) {
				const data = JSON.parse(bookmarksArrayFromStorage);
				for (var i in data) {
					content.push(data[i]);
				}
			}
	...
	async function addBookmark() {
		if (window.DPR_Globals.NavigationTabViewModel.isStorageSupportedByBrowser) {
			const bookmarksArrayFromStorage = localStorage.getItem("bookmarksArray");
			if (bookmarksArrayFromStorage) {
				const value = `${window.DPR_Globals.OtherDialogsViewModel.bookmarkName()}@${window.DPR_Globals.NavigationTabViewModel.sectionPlace}`;
				const data = JSON.parse(bookmarksArrayFromStorage);
				for (var i in data) {
	...
	async function bookmarkframe(refresh) {
		...
		for (var i in bookmList) {
			if (bookmList[i].indexOf("@") > -1) {
				...
				tbookm[1] =
					...
					tbookm[1].charAt(0) +
					"'" +
					tbookm[1].substring(1, ttbm1) +
					"'" +
					tbookm[1].charAt(ttbm1) +
					"'";
				bookmOut +=
					'<a style="color:red" href="javascript:void(0)" title="delete item" onclick="DPR_bookmarks_mod.eraseBookmark(\'' +
					i +
					'\');">x</a>&nbsp<a href="javascript:void(0)" title="Load Section" onmouseup="DPR1_send_mod.openPlace(' +
					`${sectionId}, ` +
					"[" +
					tbookm[1] +
					'],null,null,DPR1_send_mod.eventSend(event))">' +
					tbookm[0].replace(/ /g, "&nbsp;") +
					"</a><br />";
			}
		}
		if (!bookmOut) {
			...
			isClearBm =
				'&nbsp;<a style="color:' +
				DPR_G.DPR_prefs["colsel"] +
				'" href="javascript:void(0)" title="Clear Bookmarks" onclick="DPR_bookmarks_mod.clearBookmarks()"><b>clear</b></a>';
		}
		...
		for (var i in histList) {
			if (histList[i].indexOf("@") > -1) {
				...
				thist[1] =
					...
					thist[1].charAt(0) +
					"'" +
					thist[1].substring(1, tt1) +
					"'" +
					thist[1].charAt(tt1) +
					"'";
				histOut +=
					'<a style="color:red" href="javascript:void(0)" title="delete item" onclick="DPR_history_mod.removeHistory(\'' +
					i +
					'\');">x</a>&nbsp<a href="javascript:void(0)" title="Load Section" onmouseup="DPR1_send_mod.openPlace(' +
					`${sectionId}, ` +
					"[" +
		...
		if (!histOut) {
			...
			isclearHist =
				'&nbsp;<a style="color:' +
				DPR_G.DPR_prefs["colsel"] +
				'" href="javascript:void(0)" title="Clear History" onclick="DPR_history_mod.clearHistory()"><b>clear</b></a>';
		}

		if (refresh === 1) {
			await DPRSend.importXML(
				false,
				null,
				null,
				null,
		...
		$("#paliTextContent").html(
			'<table width="100%"><tr><td><span class="huge">Bookmarks</span>' +
				isClearBm +
				'</td><td width="1"></td><td><span class="huge">History</span> ' +
				isclearHist +
				'</td></tr><tr><td valign=top><div class="round">' +
				bookmOut +
				'</div></td><td></td><td width="1" valign=top><div class="round">' +
				histOut +
				"</div></td></tr></table>",
		);
	...
	async function bookmarkxd(desc, idx) {
		...
		var bk = xmlDoc
			...
			[idx].getElementsByTagName("description")[0];
		bk.textContent = desc;
	...
	async function bookmarkxn(name, idx) {
		...
		var bk = xmlDoc
			.getElementsByTagName("bookmark")
			[idx].getElementsByTagName("name")[0];
		bk.textContent = name;
	...
	function convertOldBookmarks() {
		...
		for (var i = 0; i < ca.length; i++) {
			if (/^DPB/.exec(ca[i])) {
				...
				ba[name]["loc"] = ba[name]["loc"].replace(
					/^([0-9]+)/,
					DPR_G.G_numberToNik[nik],
				);
			} else if (/^DPD/.exec(ca[i])) {
				if (!ba[DPR_translit_mod.toUni(ca[i].substring(3))]) continue;
				ba[DPR_translit_mod.toUni(ca[i].substring(3))]["desc"] =
					DPR_io_mod.readFile(ca[i])[0];
			} else if (/^DPS/.exec(ca[i])) {
				if (!ba[DPR_translit_mod.toUni(ca[i].substring(3))]) continue;
				ba[DPR_translit_mod.toUni(ca[i].substring(3))]["scroll"] =
					DPR_io_mod.readFile(ca[i])[0];
			}
		}
