const DPR_Convert = (() => {
	...
	function convert(check, lower) {
		...
		if (check && spell == DPR_G.G_oldConvert) return;
		...
		if (lower) spell = spell.toLowerCase();
	...
	function changeConversion(inout) {
		...
		if (ii == oi) {
			if (inout == 1) {
				if (ii == 0) document.getElementById("coutform").selectedIndex = 1;
				else document.getElementById("coutform").selectedIndex = 0;
			} else {
				if (oi == 0) document.getElementById("cinform").selectedIndex = 1;
				else document.getElementById("cinform").selectedIndex = 0;
			}
		}

		$("#cinput-container").attr(
			"script",
			DPR_translit_mod.getConvertLangId(ii),
		);
		$("#coutput-container").attr(
			"script",
			DPR_translit_mod.getConvertLangId(oi),
		);
		convert();
	}
	function clearboth() {
	...
	function sendtoPad(data, shift) {
		...
		document.getElementById("pad").value =
			(shift ? document.getElementById("pad").value : "") + data + "\n";
	}

	function clearPad() {
		if (confirm("Are you sure you want to erase all text from the text pad?"))
			document.getElementById("pad").value = "";
	}
