const DPR_Analysis_Declension = (() => {
	...
	function makeDeclensions(oneword, lastpart, nextpart) {
		...
		for (
			var k = 1;
			k < oneword.length;
			k++ //define sub-cut for analysis (length-k,oneword.length) and remaining string (0,length-k)
		) {
		...
		for (var i in DPR_G.G_endings) {
			...
			if (endings[gend[0]] && endings[gend[0]] > gend[2]) {
				...
				if (
					!DPR1_analysis_function_mod.isUncomp(oneword, lastpart, nextpart) &&
					!wtrDup[dec]
				) {
					//wtrDup[dec] = 1;
					...
					if (gend[4] == "v") {
						...
						wtrN.push([dec, i]);
						if (/[aiu]/.exec(dec.charAt(dec.length - 1))) {
							// long vowels
			...
			for (var stem in DPR_G.G_altStem) {
				...
				if (
					endings[stem + gend[0]] &&
					endings[stem + gend[0]] > gend[2] &&
					((DPR_G.G_altStem[stem][1] && gend[4] == "v") ||
						(!DPR_G.G_altStem[stem][1] && gend[4] == "n"))
				) {
					for (var gas0 in DPR_G.G_altStem[stem][0]) {
						var dec =
							oneword.substring(0, endings[stem + gend[0]]) +
							DPR_G.G_altStem[stem][0][gas0];
