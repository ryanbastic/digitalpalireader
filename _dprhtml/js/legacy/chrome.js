const DPR_Chrome_Tabs = (() => {
	function openFirstDPRTab() {
		if (!findDPRTab("DPR-main"))
			openDPRMain(
				"DPR-main",
				DPR_PAL.toWebUrl("chrome://digitalpalireader/content/index.xul"),
				"",
			);
	}
	...
	function openDPRTab(permalink, id, reuse) {
		...
		if (reuse) {
			...
			DPR_PAL.mainWindow.gBrowser.getBrowserForTab(
				oldTab,
			).contentDocument.location.href = permalink;
		} else {
			// get last DPR tab
			...
			for (
				var index = 0, tabbrowser = DPR_PAL.mainWindow.gBrowser;
				...
				var ctloc =
					DPR_PAL.mainWindow.gBrowser.getBrowserForTab(currentTab)
						.contentDocument.location.href;
				if (
					!/^DPR/.exec(currentTab.getAttribute("id")) ||
					!DPR_PAL.dprUrlMatcher.exec(ctloc)
				) {
					// not a dpr tab
					if (start == 1) {
						// prev was a DPR tab
	...
	function findDPRTab(id, loc) {
		for (
			var found = false, index = 0, tabbrowser = DPR_PAL.mainWindow.gBrowser;
			...
			var currentTab = tabbrowser.tabContainer.childNodes[index];
			var ctloc =
				DPR_PAL.mainWindow.gBrowser.getBrowserForTab(currentTab).contentDocument
					.location.href;

			// Does this tab contain our custom attribute?
			if (
				currentTab.getAttribute("id") == id &&
				DPR_PAL.dprUrlMatcher.exec(ctloc)
			) {
				return currentTab;
			}
	...
	function isDPRTab(id) {
		if (DPR_PAL.mainWindow.gBrowser.selectedTab.id == id)
			return DPR_PAL.mainWindow.gBrowser.selectedTab;
		else return false;
	}

	function giveIDtoTabs() {
		// startup function, give ids to
		if (!DPR_PAL.isXUL) {
			return false;
		}
		...
		var etc = 0;
		for (
			var index = 0, tb = DPR_PAL.mainWindow.gBrowser;
			index < tb.tabContainer.childNodes.length;
			index++
		) {
			// Get the next tab
			var currentTab = tb.tabContainer.childNodes[index];
			var ctloc = tb.getBrowserForTab(currentTab).contentDocument.location.href;
			if (DPR_PAL.dprUrlMatcher.exec(ctloc)) {
				// a dpr tab
				tb.setIcon(
					currentTab,
					"chrome://digitalpalireader/skin/icons/logo.png",
				);
				if (/index\.xul/.exec(ctloc))
					currentTab.setAttribute("id", main++ == 0 ? "DPR-main" : "DPRm");
				else if (/dict\.htm/.exec(ctloc))
					currentTab.setAttribute("id", dict++ == 0 ? "DPR-dict" : "DPRd");
				else if (/search\.xul/.exec(ctloc))
					currentTab.setAttribute("id", search++ == 0 ? "DPR-search" : "DPRs");
	...
	function checkLastTab() {
		...
		for (
			...
			var currentTab = tabbrowser.tabContainer.childNodes[index];
			var ctloc =
				DPR_PAL.mainWindow.gBrowser.getBrowserForTab(currentTab).contentWindow
					.location.href;
			if (
				/^DPR/.exec(currentTab.getAttribute("id")) &&
				DPR_PAL.dprUrlMatcher.exec(ctloc)
	...
	function DPRSidebarWindow() {
		...
		if (
			sidebar.contentDocument.location.href ==
			DPR_PAL.toWebUrl(
				"chrome://digitalpalireader/content/digitalpalireader.xul",
			)
		) {
	...
	function DPRSidebarDocument() {
		...
		var sidebar =
			DPR_PAL.mainWindow.document.getElementById("sidebar").contentDocument;

		if (
			sidebar.location.href ==
			DPR_PAL.toWebUrl(
				"chrome://digitalpalireader/content/digitalpalireader.xul",
			)
		) {
	...
	function toggleDPRSidebar() {
		window.DPR_Globals.DprViewModel.sidebarVisible(
			!window.DPR_Globals.DprViewModel.sidebarVisible(),
		);
		DPR_prefload_mod.saveSideBarVisibleState(
			window.DPR_Globals.DprViewModel.sidebarVisible(),
		);
	}
	...
	function closeBrowser(id) {
		...
		var thisDocument =
			DPR_PAL.mainWindow.gBrowser.getBrowserForTab(thisTab).contentDocument;
		thisDocument
			.getElementById(id)
			.parentNode.removeChild(thisDocument.getElementById(id + "-splitter"));
		thisDocument
			.getElementById(id)
			.parentNode.removeChild(thisDocument.getElementById(id));
	}
	...
	function DPRBottomPaneUpdateStyle() {
		...
		document.getElementById("bottom").style.top =
			document.getElementById("anf").offsetHeight - 4 + "px";
	}

	function DPRShowBottomPane(tabIdToActivate = "D") {
		window.DPR_Globals.BottomPaneTabsViewModel.updateActiveTabId(
			tabIdToActivate,
		);
		openBottomFrame();
	}
	...
	const writeNavigationHeaderForSection = (
		sectionId,
		titleout0,
		modt,
		range,
		...
		$(
			...
			modt +
				...
				titleout0 +
				(range
					? ' <span class="tiny">para. ' + range.join("-") + "</span>"
					: "") +
				(place8 ? '<span class="tiny">(Thai)</span>' : "") +
				`</nav>`,
		);
	};
...
const DPR_Chrome_UI = (() => {
	...
	const fixupUrlAndMainPanelSectionsLayout = () => {
		...
		const totalSplitterWidth = sectionElements.splitters.reduce(
			(acc, e) => acc + e.width(),
			0,
		);
		sectionElements.sections.forEach((x) =>
			x.width(
				(availableWidth - totalSplitterWidth) / sectionElements.sections.length,
			),
		);

		const uris = sectionElements.sections
			.map((x) => x.attr("data-dpruri"))
			.filter((x) => x)
			.slice(1);
	...
	const splitterDragHandler = (sPos) => (_, element, newWidth, __, ___) => {
		...
		const newNextSectionWidth =
			sectionElements.sections[sPos + 1].width() - delta;
	...
	const getPrimarySectionElementId = () =>
		getSectionElementId(getPrimarySectionId());

	const isPrimarySectionId = (id) =>
		Number.parseInt(id) === getPrimarySectionId();

	const paneCommandDataKeyName = "data-dprcommands";

	const getPaneCommandData = (id) =>
		JSON.parse($(getSectionElementId(id)).attr(paneCommandDataKeyName) || "{}");

	const setPaneCommandData = (id, data) => {
		$(getSectionElementId(id)).attr(
			paneCommandDataKeyName,
			JSON.stringify(data),
		);
    ...
    <div class="main-pane-container-section" id="${getSectionElementIdName(sPos)}" ${paneCommandDataKeyName}="{}" data-dpruri="${DPR_Translations.makeUri(sInfo)}" style="background: ${DPR_Translations.getTrProps()[sInfo.type].background}">
      ...
      ${content}
		...
		window.dispatchEvent(
			new CustomEvent("DPR_Swipe_Gesture:touchstart", {
				detail: {
					sectionElementId: getSectionElementIdName(sPos),
				},
			}),
		);
		window.dispatchEvent(
			new CustomEvent("DPR_Swipe_Gesture:touchmove", {
				detail: {
					sectionElementId: getSectionElementIdName(sPos),
				},
			}),
		...
		window.dispatchEvent(
			new CustomEvent("DPR_Swipe_Gesture:touchend", {
				detail: {
					sectionElementId: getSectionElementIdName(sPos),
					sectionPosition: sPos,
				},
			}),
		);
	};
	...
	const goPreviousInSecondaryPane = async (id) => {
		...
		await DPR1_send_mod.openPlace(
			id,
			[...cmdData.prev, cmdData.place[8] ? 1 : ""],
			null,
			null,
			"internal",
		);
	...
	const goNextInSecondaryPane = async (id) => {
		...
		await DPR1_send_mod.openPlace(
			id,
			[...cmdData.next, cmdData.place[8] ? 1 : ""],
			null,
			null,
			"internal",
		);
	...
	const loadTranslationSection = (sInfo, sectionId) => {
		$(
			`${getSectionElementId(sectionId)} .main-pane-container-section-iframe`,
		).attr("src", DPR_Translations.resolveUri(sInfo));
	};

	const addOrOpenMainPanelSection = async (
		sInfo,
		fixupUrlAndLayout = true,
		...
		if (sectionId === null) {
			...
			sectionId = Number.parseInt(
				$("#main-pane-container").attr("data-nextsectionid"),
			);
		}
	...
	const addMainPanelSections = async (
		sInfos,
		section0Id,
		section0Query,
		section0Para,
		...
		for (let i = 0; i < sInfos.length; i++) {
			...
			await addOrOpenMainPanelSection(
				sInfos[i],
				false,
				targetSectionId,
				section0Query,
	...
	const createToast = (
		...
		delay,
		uniqueId,
		{ toastCommandName, toastCommandHandler, toastCommandLink },
	) => {
		...
		if (uniqueId && $(containerId).find(`#${uniqueId}`).length) {
			console.log(
				...
				uniqueId,
				"already exists. Not creating another.",
			);
	...
	const showErrorToast = (
		message,
		toastVisibleForMilliseconds = defaultToastVisibleForMilliseconds,
	) =>
		createToast(ToastTypeError, message, toastVisibleForMilliseconds, null, {});
	...
	const showSingletonInformationToast = (
		message,
		uniqueId,
		toastVisibleForSeconds,
		toastCommandInfo,
	) =>
		createToast(
			ToastTypeInfo,
			message,
			toastVisibleForSeconds * 1000,
			uniqueId,
			toastCommandInfo,
		);
