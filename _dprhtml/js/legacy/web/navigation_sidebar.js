const DPR_Web_Navigation_Sidebar = (() => {
	async function changeSet(nik) {
		window.DPR_Globals.NavigationTabViewModel.set(nik);

		const prevSet =
			DPR_G.G_numberToNik[
				window.DPR_Globals.NavigationTabViewModel.prevSetIndex
			];
		if (
			DPR_G.G_hier == "t" &&
			this.limitt(
				DPR_G.G_nikToNumber2[window.DPR_Globals.NavigationTabViewModel.set()],
			)
		) {
			DPR_Chrome.showErrorToast(
				"Ṭīkā not available for " +
					DPR_G.G_nikLongName[window.DPR_Globals.NavigationTabViewModel.set()] +
					".",
			);
		...
		if (
			DPR_G.G_hier == "a" &&
			window.DPR_Globals.NavigationTabViewModel.set() == "g"
		) {
		...
		if (
			DPR_G.G_hier == "a" &&
			window.DPR_Globals.NavigationTabViewModel.set() == "b"
		) {
		...
		window.DPR_Globals.NavigationTabViewModel.prevSetIndex =
			DPR_G.G_nikToNumber2[nik];

		this.setBookList(nik);
	}

	function getBookName(nik, ht, no) {
		// nik is nikaya, ht is a DPR_G.G_hier, no will be xml no - 1
	...
	function setBookList(nik, book) {
		...
		for (var i = 0; i < titles.length; i++) {
			...
			window.DPR_Globals.NavigationTabViewModel.navBook.push({
				value: val,
				label: DPR_translit_mod.translit(title),
			});
		}

		window.DPR_Globals.NavigationTabViewModel.book(
			book
				? book
				: window.DPR_Globals.NavigationTabViewModel.navBook()[0].value,
		);
	}
	...
	async function setSearchBookList() {
		...
		const titles = DPR_G.nikvoladi[nik]
			? DPR_G.nikvoladi[nik]
			: DPR_G.nikvoladi[nik + $("#tsoMAT2m").val()];
		...
		for (var i = 0; i < titles.length; i++) {
			// menu
			const menuValue =
				nik == "k" || nik == "y" || nik == "n" ? titles[i] + 1 : i + 1;
			const menuText = DPR_translit_mod.translit(
				nik == "k" || nik == "y" || nik == "n"
					? DPR_G.G_kynames[nik][titles[i]]
					: DPR_G.G_nikLongName[nik] + " " + titles[i],
			);

			window.DPR_Globals.SearchTabViewModel.bookMenu.push({
				label: menuText,
				value: menuValue,
			});
			...
			const label =
				nik == "k" || nik == "y" || nik == "n"
					? DPR_G.G_kynames[nik][titles[i]]
					: (typeof titles[i] == "number" ? "Book " : "") + titles[i];
			const cbValue =
				nik == "k" || nik == "y" || nik == "n" ? titles[i] + 1 : i + 1;

			if (i >= Math.ceil(titles.length / 2)) {
				window.DPR_Globals.SearchTabViewModel.bookListB.push({
					label: label,
					id: `tsoBObook${i + 1}`,
					value: cbValue,
					selected: window.DPR_Globals.SearchTabViewModel.searchBookCheckbox(
						i + 1,
					),
				});
			} else {
				window.DPR_Globals.SearchTabViewModel.bookListA.push({
					label: label,
					id: `tsoBObook${i + 1}`,
					value: cbValue,
					selected: window.DPR_Globals.SearchTabViewModel.searchBookCheckbox(
	...
	function switchhier(htmp) {
		...
		if (
			htmp == "t" &&
			this.limitt(window.DPR_Globals.NavigationTabViewModel.prevSetIndex)
		) {
			DPR_Chrome.showErrorToast(
				"Ṭīkā not available for " +
					DPR_G.G_nikLongName[window.DPR_Globals.NavigationTabViewModel.set()] +
			...
			window.DPR_Globals.NavigationTabViewModel.MAT(
				window.DPR_Globals.NavigationTabViewModel.prevMat,
			);
			return;
		}
		if (
			htmp == "a" &&
			window.DPR_Globals.NavigationTabViewModel.prevSetIndex > 7
		) {
			DPR_Chrome.showErrorToast(
				"Aṭṭhakathā not available for " +
					DPR_G.G_nikLongName[window.DPR_Globals.NavigationTabViewModel.set()] +
					".",
			);
			window.DPR_Globals.NavigationTabViewModel.MAT(
		...
		if (
			window.DPR_Globals.NavigationTabViewModel.set() == "k" &&
			htmp == "a" &&
			DPR_G.kudvala[window.DPR_Globals.NavigationTabViewModel.book()] ==
				undefined
		) {
			DPR_Chrome.showErrorToast(
				"Aṭṭhakathā not available for " +
					this.getBookName(
						window.DPR_Globals.NavigationTabViewModel.set(),
						htmp,
						window.DPR_Globals.NavigationTabViewModel.navBook().findIndex(
							(x) =>
								x.value === window.DPR_Globals.NavigationTabViewModel.book(),
						),
					) +
					".",
			);
		...
		} else book = Number.parseInt(book) - 1;
	...
	function historyBox() {
		...
		if (theHistory.length > 0) {
			...
			for (var i = 0; i < theHistory.length; i++) {
				...
				thist0 = thist0.split("-");
				thist0[thist0.length - 1] = DPR_translit_mod.translit(
					thist0[thist0.length - 1],
				);
				thist0 = thist0.join("-");
				...
				if (x.length > 3) {
					ch[i + 1].setAttribute(
						...
						"DPRSend.openPlace(" +
							...
							",'" +
							x[7] +
							"'],null,null,DPRSend.eventSend(event));",
					);
				} else {
					ch[i + 1].setAttribute(
						"onclick",
	...
	function searchHistoryBox() {
		...
		if (bNodes.length > 0) {
			...
			for (var i = bNodes.length - 1; i >= 0; i--) {
				...
				var searchType =
					bNodes[i].getElementsByTagName("searchType")[0].textContent;
				var searchString =
					bNodes[i].getElementsByTagName("query")[0].textContent;
				...
				switch (Number.parseInt(searchType)) {
					case 0:
					case 5:
					...
					case 4:
						place =
							DPR_G.G_nikLongName[searchSet] +
							" " +
							searchBook.split("").join(",");
						break;
				}
				...
				bList.appendItem(
					"'" +
						...
						place +
						" (" +
						types[Number.parseInt(searchType)] +
						")",
				);
	...
	function dictHistoryBox() {
		...
		if (bNodes.length > 0) {
			...
			for (var i = bNodes.length - 1; i >= 0; i--) {
				...
				ch[cnt].setAttribute(
					"onclick",
					"DPRSend.sendDict(true,DPRSend.eventSend(event),'" +
						type +
						"','" +
						DPR_translit_mod.translit(query) +
						"',['" +
						opts +
						"']);",
				);
	...
	function gotoPlace(place) {
		...
		const b = window.DPR_Globals.NavigationTabViewModel.navBook().findIndex(
			(x) => x.value === Number.parseInt(place[1]) + 1,
		);
		window.DPR_Globals.NavigationTabViewModel.book(
			window.DPR_Globals.NavigationTabViewModel.navBook()[b].value,
		);
		if (place.length > 3) {
			window.DPR_Globals.NavigationTabViewModel.meta(
				place[2].toString().replace("x", "0"),
			);
			window.DPR_Globals.NavigationTabViewModel.volume(
				place[3].toString().replace("x", "0"),
			);
			window.DPR_Globals.NavigationTabViewModel.vagga(
				place[4].toString().replace("x", "0"),
			);
			window.DPR_Globals.NavigationTabViewModel.sutta(
				place[5].toString().replace("x", "0"),
			);
			window.DPR_Globals.NavigationTabViewModel.section(
				place[6].toString().replace("x", "0"),
			);
		}
	}
