const DPR_Web_Xml_Sidebar = (() => {
	async function updateHierarchy(depth) {
		// depth: 4=section, 3=sutta..., 2=vagga..., 1=volume..., 0=all
		...
		var volume =
			depth > 1 ? window.DPR_Globals.NavigationTabViewModel.volume() : 0;
		var vagga =
			depth > 2 ? window.DPR_Globals.NavigationTabViewModel.vagga() : 0;
		var sutta =
			depth > 3 ? window.DPR_Globals.NavigationTabViewModel.sutta() : 0;

		var axml, name, namea;

		axml = xmlDoc.getElementsByTagName("ha");
		namea = axml[0].getElementsByTagName("han");
		if (namea[0].childNodes[0] && namea[0].textContent.length > 1)
			name = namea[0].textContent
				.replace(/\{.*\}/, "")
				.replace(/^ +/, "")
				.replace(/ +$/, "");
		...
		switch (true) {
			case depth < 1: // remake meta list
				...
				this.updateDepth(
					u,
					window.DPR_Globals.NavigationTabViewModel.navMeta,
					"h0n",
				);
			// fall through.
			...
			case depth < 2: // remake volume list
				...
				this.updateDepth(
					v,
					window.DPR_Globals.NavigationTabViewModel.navVolume,
					"h1n",
				);
			// fall through.
			...
			case depth < 3: // remake vaggalist
				...
				this.updateDepth(
					w,
					window.DPR_Globals.NavigationTabViewModel.navVagga,
					"h2n",
				);
			// fall through.
			...
			case depth < 4: // remake sutta list on depth = 0, 1, 2, or 3
				...
				this.updateDepth(
					x,
					window.DPR_Globals.NavigationTabViewModel.navSutta,
					"h3n",
				);
			// fall through.
			...
			case depth < 5: // remake section list
				...
				this.updateDepth(
					y,
					window.DPR_Globals.NavigationTabViewModel.navSection,
					"h4n",
				);
				break;
	...
	async function updateSearchHierarchy(depth) {
		// depth: 4=section, 3=sutta..., 2=vagga..., 1=volume..., 0=all
		...
		if (xmlDoc === null) {
			...
			DPR_prefload_mod.saveSearchSettings(
				DPR_G.DPR_prefsinfo[DPR_prefload_mod.searchSettingsKeyName]
					.defaultValue,
			);
			return;
		}
		...
		switch (true) {
			case depth == 0: // remake meta list
				...
				if (lista.length == 1 && lista[0] == this.unnamed) {
					window.DPR_Globals.SearchTabViewModel.metaList.push({
						label: this.unnamed,
						value: 0,
					});
					$("#tsoP1").hide();
			...
			case depth < 2: // remake volume list
				...
				if (lista.length == 1 && lista[0] == this.unnamed) {
					window.DPR_Globals.SearchTabViewModel.volumeList.push({
						label: this.unnamed,
						value: 0,
					});
					$("#tsoP2").hide();
			...
			case depth < 3: // remake vaggalist
				...
				if (lista.length == 1 && lista[0] == this.unnamed) {
					window.DPR_Globals.SearchTabViewModel.vaggaList.push({
						label: this.unnamed,
						value: 0,
					});
					$("#tsoP3").hide();
			...
			case depth < 4: // remake sutta list on depth = 0, 2, or 3
				...
				if (lista.length == 1 && lista[0] == this.unnamed) {
					window.DPR_Globals.SearchTabViewModel.suttaList.push({
						label: this.unnamed,
						value: 0,
					});
					$("#tsoP4").hide();
			...
			default: // remake section list
				...
				if (lista.length == 1 && lista[0] == this.unnamed) {
					window.DPR_Globals.SearchTabViewModel.sectionList.push({
						label: this.unnamed,
						value: 0,
					});
					$("#tsoP5").hide();
		...
		if (
			$("#tsoPart input[name=tsoPR]:visible").length > 0 &&
			$("#tsoPart input[name=tsoPR]:visible:checked").length == 0
		) {
			window.DPR_Globals.SearchTabViewModel.partialValue(
				$("#tsoPart input[name=tsoPR]:visible:first")[0].value,
			);
		}
	}

	function makeTitleSelect(xml, tag) {
		// output menupopup tag with titles in menuitems
		var name, namea;
		var outlist = [];
		for (var a = 0; a < xml.length; a++) {
			...
			if (
				name[0].childNodes[0] &&
				name[0].textContent.replace(/ /g, "").length > 0
			)
				namea = name[0].textContent
					.replace(/\{.*\}/, "")
					.replace(/^ +/, "")
					.replace(/ +$/, "");
			else {
