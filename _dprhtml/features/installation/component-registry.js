...
export const registry = [
	{
		...
		capture: ({ url }) =>
			...
			/^\/tipitaka\/my\//i.test(url.pathname),
		isAvailable: () => true,
	...
	},
	{
		...
		capture: ({ url }) =>
			url.origin === self.location.origin &&
			/^\/tipitaka\/th\//i.test(url.pathname),
		isAvailable: () => true,
		type: componentTypeTipitaka,
		sizeMB: 21,
		getFileList: async () => {
	...
	},
	{
		...
		capture: ({ url }) =>
			url.origin === self.location.origin && /^\/en\//i.test(url.pathname),
		isAvailable: () => true,
	...
	},
	{
		...
		capture: ({ url }) =>
			url.origin === self.location.origin && /^\/sa\//i.test(url.pathname),
		isAvailable: () => true,
		type: componentTypeLanguage,
		sizeMB: 94,
		getFileList: async () => {
	...
	},
	{
		...
		capture: ({ url }) =>
			url.origin === self.location.origin &&
			/^\/_external\/translations\/bt-/i.test(url.pathname),
		isAvailable: () => true,
		type: componentTypeTranslation,
		sizeMB: 53,
		getFileList: async () => {
			const files = await import(
				"../../../components/translation/bt/bt_list.js"
			);
			return files.default.map(
				(x) => `${DprGlobals.singleton.BTTranslationsBaseUrl}/${x}`,
			);
		},
	},
	{
		...
		capture: ({ url }) =>
			url.origin === self.location.origin &&
			/^\/_external\/translations\/dt\//i.test(url.pathname),
		isAvailable: () => true,
		type: componentTypeTranslation,
		sizeMB: 22,
		getFileList: async () => {
			const files = await import(
				"../../../components/translation/dt/dt_list.js"
			);
			return files.default.map(
				(x) => `${DprGlobals.singleton.DTTranslationsBaseUrl}/${x}`,
			);
		},
	},
	{
		...
		capture: ({ url }) =>
			url.origin === self.location.origin &&
			/^\/_external\/translations\/ati\//i.test(url.pathname),
		isAvailable: () => true,
		type: componentTypeTranslation,
		sizeMB: 6,
		getFileList: async () => {
			const files = await import(
				"../../../components/translation/ati/ati_list.js"
			);
			return files.default.map(
				(x) => `${DprGlobals.singleton.ATITranslationsBaseUrl}/${x}`,
			);
		},
	},
];
...
export const componentInstallDoneMarkerKeyName = (id) =>
	`Component: ${getComponentFromId(id).name} installed`;

export const isComponentInstalled = (id) =>
	!!localStorage[componentInstallDoneMarkerKeyName(id)];

export const getComponentCacheName = (id) => {
	const prefix =
		getComponentFromId(id).type === componentTypeLanguage
			? "lang"
			: getComponentFromId(id).type;
...
const componentToVM = (c) => {
	...
	return {
		...
		getName: (_) =>
			`${c.name} ${c.type}${description} [download ${Math.ceil(c.sizeMB * 0.2)}MB, uncompressed on disk ${c.sizeMB}MB]`,
	};
};

export const getAvailableComponentVMs = () =>
	registry.filter((c) => c.isAvailable()).map(componentToVM);
