const DPR_Swipe_Gesture = (() => {
	...
	window.addEventListener("DPR_Swipe_Gesture:touchstart", (evt) => {
		document
			.getElementById(evt.detail.sectionElementId)
			.addEventListener("touchstart", DPR_Gesture.touchStart, true);
	});
	window.addEventListener("DPR_Swipe_Gesture:touchmove", (evt) => {
		document
			.getElementById(evt.detail.sectionElementId)
			.addEventListener(
				"touchmove",
				DPR_Gesture.touchMoveFactory(evt.detail.sectionElementId),
				true,
			);
	});

	window.addEventListener("DPR_Swipe_Gesture:touchend", (evt) => {
		document
			.getElementById(evt.detail.sectionElementId)
			.addEventListener(
				"touchend",
				DPR_Gesture.touchEndFactory(
					evt.detail.sectionElementId,
					evt.detail.sectionPosition,
				),
				true,
			);
	});
	...
	const touchEndFactory = (sectionElementId, sectionPosition) => {
		return (event) => {
			if (startX || startY) {
				...
				const distance =
					(Math.sqrt(endX * endX + endY * endY) / window.innerWidth) * 100;
				...
				const horizontalToVerticalRatio = Math.log2(
					Math.abs(swipeXDiff / swipeYDiff),
				);
	...
	const matchesSwipeCriteria = (horizontalToVerticalRatio, speed) => {
		const validSpeedSkew =
			horizontalToVerticalRatio > (1 - allowedSkew) * swipeRatioThreshold &&
			speed > (1 + allowedSkew) * speedThreshold;
		const validRatioSkew =
			horizontalToVerticalRatio > (1 + allowedSkew) * swipeRatioThreshold &&
			speed > (1 - allowedSkew) * speedThreshold;
		const validNoSkew =
			horizontalToVerticalRatio > swipeRatioThreshold && speed > speedThreshold;
	...
	const applyMoveEffects = (section, swipeXDiff, swipeYDiff) => {
		if (
			Math.abs(swipeXDiff) > minSwipeX &&
			Math.abs(swipeXDiff) < 2 * minSwipeX
		) {
	...
	const runMatchingCommand = (e) => {
		const cmd = Object.entries(window.DPR_Globals.DprViewModel.commands).find(
			([_, x]) => x().matchGesture(e),
		);
		if (
			cmd &&
