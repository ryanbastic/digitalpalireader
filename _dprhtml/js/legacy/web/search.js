const DPR_Web_Search = (() => {
  ...
  <li>
    <a href="#" onclick="return DPR1_search_mod.scrollSearch()">${DPR_translit_mod.translit(searchTerm)}</a>
		...
		DPR_G.MD.getElementById("search-sets").insertAdjacentHTML(
			"beforeend",
			html,
		);
	}
  ...
  `;
	...
	function fixPluralInSearchTermSectionInfo() {
		var val = Number.parseInt(
			DPR_G.MD.getElementById("search-matches").innerHTML,
		);
		if (val == 1) {
			var str = DPR_G.MD.getElementById("inter").innerHTML.replace(
				"matches",
				"match",
			);
  ...
  `;
		DPR_G.MD.getElementById("search-link").insertAdjacentHTML(
			"beforeend",
			html,
		);
	}
